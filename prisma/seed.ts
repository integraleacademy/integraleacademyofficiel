...
async function main() {
  ...
  try {
    ...
    for (const session of a3pSeedSessions) {
      await prisma.trainingSession.upsert({
        where: { id: session.id },
        update: { trainingId: a3p.id, title: session.title, startDate: new Date(session.startDate), endDate: new Date(session.endDate), examDate: new Date(session.examDate), priceCents: 420000, priceLabel: '4 200 €', location: 'Puget-sur-Argens / Côte d’Azur', status: 'OPEN', seatsLeft: session.seatsLeft, showSeatsLeft: true, durationLabel: '328 heures hors examen', registrationUrl: '/formations-securite/a3p-apr', fundingNotes: 'Financement possible selon votre situation : CPF, France Travail ou financement personnel à vérifier avec l’équipe.', publicNotes: 'Formation de 328 heures hors examen, dont 92 heures et 50 minutes de pratique ; hébergement collectif possible sur réservation.', sortOrder: session.sortOrder, isHighlighted: session.sortOrder === 0 },
