...
const emptySession = (training?: TrainingRow) => ({ trainingId: training?.id || '', title: '', startDate: '', endDate: '', examDate: '', priceCents: '', priceLabel: '', location: '', status: 'OPEN', seatsTotal: '', seatsLeft: '', showSeatsLeft: true, durationLabel: '', registrationUrl: '', fundingNotes: '', publicNotes: '', internalNotes: '', sortOrder: 0, isHighlighted: false });

function serialize(row: SessionRow) {
  return { trainingId: row.trainingId, title: row.title, startDate: inputDate(row.startDate) || row.startDate, endDate: inputDate(row.endDate) || row.endDate, examDate: inputDate(row.examDate) || row.examDate, priceCents: row.priceCents, priceLabel: row.priceLabel, location: row.location || '', status: row.status, seatsTotal: row.seatsTotal ?? '', seatsLeft: row.seatsLeft ?? '', showSeatsLeft: row.showSeatsLeft !== false, durationLabel: row.durationLabel || '', registrationUrl: row.registrationUrl || '', fundingNotes: row.fundingNotes || '', publicNotes: row.publicNotes || '', internalNotes: row.internalNotes || '', sortOrder: row.sortOrder || 0, isHighlighted: !!row.isHighlighted };
}
...
export function SessionsClient({ initialRows }: { initialRows: SessionRow[] }) {
  ...
  function updateDraft(patch: Record<string, any>) { setDraft((current: any) => ({ ...(current || emptySession(trainings[0])), ...patch })); }
  ...
  function addSession() {
    ...
    setDraft(emptySession(trainings[0]));
  }
  ...
  const fields = (row: SessionRow, onChange: (patch: Record<string, any>) => void, disabled: boolean) => <>
    <label className="text-sm font-semibold">Formation<select value={row.trainingId || ''} onChange={event => onChange({ trainingId: event.target.value })} className="mt-1 w-full rounded-xl border p-3" disabled={disabled}>{trainings.map(training => <option key={training.id} value={training.id}>{training.name}</option>)}</select></label>
    ...
    <label className="text-sm font-semibold">Places restantes affichées<input type="number" min="0" value={row.seatsLeft ?? ''} onChange={event => onChange({ seatsLeft: event.target.value })} className="mt-1 w-full rounded-xl border p-3" disabled={disabled}/><span className="mt-1 block text-xs font-normal text-stone-500">Laissez vide pour utiliser la règle automatique J-60/J-45/J-30/J-15.</span></label>
    <label className="text-sm font-semibold">Afficher les places restantes<select value={row.showSeatsLeft === false ? 'no' : 'yes'} onChange={event => onChange({ showSeatsLeft: event.target.value === 'yes' })} className="mt-1 w-full rounded-xl border p-3" disabled={disabled}><option value="yes">Oui</option><option value="no">Non</option></select></label>
    <label className="text-sm font-semibold">Durée<input value={row.durationLabel || ''} onChange={event => onChange({ durationLabel: event.target.value })} className="mt-1 w-full rounded-xl border p-3" disabled={disabled}/></label>
  ...
  return <>
    ...
    <div className="mt-5 flex flex-wrap gap-3"><button onClick={addSession} disabled={!canEdit} className="rounded-xl bg-academy-gold px-4 py-3 font-bold disabled:cursor-not-allowed disabled:opacity-50">Ajouter</button>{health && health.ok && health.trainingsCount === 0 ? <button onClick={initialize} className="rounded-xl bg-academy-ink px-4 py-3 font-bold text-white">Initialiser les formations</button> : null}</div>
