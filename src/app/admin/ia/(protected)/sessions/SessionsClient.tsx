...
const emptySession = (training?: TrainingRow) => ({ trainingId: training?.id || '', title: '', startDate: '', endDate: '', examDate: '', remoteStartDate: '', remoteEndDate: '', onsiteStartDate: '', onsiteEndDate: '', priceCents: '', priceLabel: '', location: '', status: 'OPEN', seatsTotal: '', seatsLeft: '', registrationUrl: '', fundingNotes: '', publicNotes: '', internalNotes: '', sortOrder: 0, isHighlighted: false });

function serialize(row: SessionRow) {
  return { trainingId: row.trainingId, title: row.title, startDate: inputDate(row.startDate) || row.startDate, endDate: inputDate(row.endDate) || row.endDate, examDate: inputDate(row.examDate) || row.examDate, remoteStartDate: inputDate(row.remoteStartDate), remoteEndDate: inputDate(row.remoteEndDate), onsiteStartDate: inputDate(row.onsiteStartDate), onsiteEndDate: inputDate(row.onsiteEndDate), priceCents: row.priceCents, priceLabel: row.priceLabel, location: row.location || '', status: row.status, seatsTotal: row.seatsTotal ?? '', seatsLeft: row.seatsLeft ?? '', registrationUrl: row.registrationUrl || '', fundingNotes: row.fundingNotes || '', publicNotes: row.publicNotes || '', internalNotes: row.internalNotes || '', sortOrder: row.sortOrder || 0, isHighlighted: !!row.isHighlighted };
}
...
export function SessionsClient({ initialRows }: { initialRows: SessionRow[] }) {
  ...
  const fields = (row: SessionRow, onChange: (patch: Record<string, any>) => void, disabled: boolean) => <>
    ...
    <label className="text-sm font-semibold">Début distanciel<input type="date" value={inputDate(row.remoteStartDate)} onChange={event => onChange({ remoteStartDate: event.target.value })} className="mt-1 w-full rounded-xl border p-3" disabled={disabled}/></label>
    <label className="text-sm font-semibold">Fin distanciel<input type="date" value={inputDate(row.remoteEndDate)} onChange={event => onChange({ remoteEndDate: event.target.value })} className="mt-1 w-full rounded-xl border p-3" disabled={disabled}/></label>
    <label className="text-sm font-semibold">Début présentiel<input type="date" value={inputDate(row.onsiteStartDate)} onChange={event => onChange({ onsiteStartDate: event.target.value })} className="mt-1 w-full rounded-xl border p-3" disabled={disabled}/></label>
    <label className="text-sm font-semibold">Fin présentiel<input type="date" value={inputDate(row.onsiteEndDate)} onChange={event => onChange({ onsiteEndDate: event.target.value })} className="mt-1 w-full rounded-xl border p-3" disabled={disabled}/></label>
