    ...
    <VisualSection tone="funding"><section className="bg-academy-bg px-4 py-14 md:py-20">
      <div className="mx-auto max-w-7xl">
        <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          ...
          <p className="max-w-xl text-sm leading-7 text-academy-muted">Comparez rapidement les parcours selon leur tarif, leur durée, leur certification et les solutions de financement possibles.</p>
        </div>
              ...
              <div className="grid gap-0 lg:grid-cols-[10rem_1.3fr_.8fr]">
                ...
                <div className="p-6 md:p-8">
                  <div className="flex flex-wrap gap-2">
                    <span className="rounded-full bg-academy-bg px-3 py-1 text-xs font-black text-academy-muted">{formation.duration}</span>
                    <span className="rounded-full bg-academy-bg px-3 py-1 text-xs font-black text-academy-muted">{formation.locations}</span>
                  </div>
                  <h3 className="mt-5 text-2xl font-black tracking-tight text-academy-ink">{formation.title}</h3>
                  <p className="mt-3 max-w-3xl text-sm leading-7 text-academy-muted">{formation.short}</p>
                  <div className="mt-5 flex flex-wrap gap-3 text-sm font-semibold text-academy-muted">
                ...
                <div className="flex flex-col justify-between border-t border-academy-line bg-academy-soft p-6 md:p-8 lg:border-l lg:border-t-0">
                  <div>
                    <p className="text-xs font-black uppercase tracking-[.2em] text-academy-muted">Tarif</p>
                    <div className="mt-3 text-4xl font-black tracking-tight text-academy-ink md:text-5xl">{price.main}</div>
                    <p className="mt-2 text-sm font-bold text-academy-muted">{price.detail}</p>
                  </div>
                  <div className="mt-7 flex flex-wrap gap-2">
                    ...
                    <Link href="/contact" className="inline-flex items-center justify-center rounded-full bg-academy-surface px-5 py-3 text-sm font-black text-academy-ink ring-1 ring-academy-line transition hover:bg-academy-gold">Devis</Link>
                  </div>
                </div>
              </div>
            </article>;
      ...
      <div className="mx-auto overflow-hidden rounded-[2rem] bg-academy-surface shadow-card ring-1 ring-academy-line md:max-w-7xl">
        <div className="grid lg:grid-cols-[.8fr_1.2fr]">
          ...
          <div className="p-8 md:p-10">
            <p className="text-academy-muted">Pour les formations affichées, un conseiller peut vérifier votre éligibilité CPF, France Travail, OPCO, entreprise ou facilités de paiement, puis confirmer un devis adapté.</p>
