...
export default function Home(){
  ...
  return <>
    ...
    <VisualSection tone="security"><section id="formations-securite" className="scroll-mt-28 mx-auto max-w-7xl px-4 py-12"><SectionTitle eyebrow="1. Sécurité privée" title="Formations professionnelles aux métiers de la sécurité">Ces formations visent l’accès ou l’évolution dans les métiers de la sécurité privée, de la protection rapprochée, du management sécurité ou de la sécurité incendie.</SectionTitle><div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">{securityHighlights.map(f=><FormationCard key={f.slug} title={f.title} desc={f.short} href={f.slug} tags={['Sécurité privée',f.duration]}/>)}</div></section></VisualSection>
    <VisualSection tone="vtc"><section className="mx-auto max-w-7xl px-4 py-12"><SectionTitle eyebrow="2. Chauffeur VTC" title="Une formation dédiée au transport de personnes">Le parcours VTC est présenté à part : il ne relève ni des métiers de la sécurité privée, ni des BTS en alternance.</SectionTitle><div className="mx-auto max-w-3xl"><FormationCard title={vtcFormation.title} desc={vtcFormation.short} href={vtcFormation.slug} tags={[vtcFormation.duration,'VTC','CPF selon éligibilité']}/></div></section></VisualSection>
    <VisualSection tone="bts"><section id="bts" className="scroll-mt-28 mx-auto max-w-7xl px-4 py-12"><SectionTitle eyebrow="3. BTS en alternance" title="Diplômes BTS : un parcours long en entreprise et en centre">Les BTS sont des diplômes d’État en alternance. Ils sont volontairement séparés des formations professionnelles métiers de la sécurité privée et du VTC.</SectionTitle><div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">{btsHighlights.map(f=><FormationCard key={f.slug} title={f.title} desc={f.short} href={f.slug} tags={f.tags}/>)}</div></section></VisualSection>
