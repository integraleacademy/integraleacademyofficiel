...
export async function isAdminAuthenticated(){const c=(await cookies()).get(COOKIE)?.value; if(!c)return false; const [exp,sig]=c.split('.'); if(!exp||!sig||Number(exp)<Date.now())return false; const expected=sign(exp); const sigBuffer=Buffer.from(sig); const expectedBuffer=Buffer.from(expected); return sigBuffer.length===expectedBuffer.length && timingSafeEqual(sigBuffer,expectedBuffer);}
