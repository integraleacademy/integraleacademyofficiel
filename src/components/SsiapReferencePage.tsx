...
function CTA({ href, children, variant = 'dark', className = '' }: { href: string; children: ReactNode; variant?: 'dark' | 'gold' | 'light' | 'outline'; className?: string }) {
  const styles = {
...
function Eyebrow({ children, light = false }: { children: ReactNode; light?: boolean }) {
  return <p className={`text-[.66rem] font-black uppercase tracking-[.24em] ${light ? 'text-orange-300' : 'text-orange-700'}`}>{children}</p>;
}
...
function HeroSession({ session }: { session: any }) {
  ...
  return <aside className="rounded-[2rem] border border-white/65 bg-[#FFFDF8] p-5 text-academy-ink shadow-[0_34px_100px_rgba(0,0,0,.34)] sm:p-6">
    <div className="flex flex-wrap items-center justify-between gap-2"><span className="rounded-full bg-[#0D1725] px-3 py-1.5 text-[.62rem] font-black uppercase tracking-[.16em] text-orange-300">Prochaine session</span><span className={`rounded-full border px-3 py-1.5 text-[.68rem] font-black ${full ? 'border-stone-300 bg-stone-100 text-stone-700' : 'border-orange-300 bg-orange-50 text-orange-800'}`}>{seatsLabel(session)}</span></div>
    <h2 className="mt-5 text-3xl font-black tracking-[-.04em] sm:text-4xl">{formatDate(session?.startDate)} <span className="text-orange-600">→</span><br />{formatDate(session?.endDate)}</h2>
    ...
    <CTA href={sessionHref(session)} variant={full ? 'light' : 'dark'} className="mt-5 w-full">{full ? 'Être alerté de la prochaine session' : 'Réserver ma place →'}</CTA>
    <CompactAssistant />
  </aside>;
}
...
export function SsiapReferencePage({ sessions }: { sessions: any[] }) {
  ...
  return <main className="relative overflow-hidden pb-24 lg:pb-0">
    ...
    <section className="relative isolate overflow-hidden bg-[#0D1725] px-4 pb-8 pt-10 text-white sm:pt-14 lg:pt-16">
      <div className="absolute inset-0 -z-10 bg-[radial-gradient(circle_at_12%_12%,rgba(238,184,47,.24),transparent_31%),radial-gradient(circle_at_88%_20%,rgba(239,68,68,.22),transparent_29%),linear-gradient(135deg,#080D15_0%,#121B2A_55%,#2A160F_100%)]" />
      <div className="absolute -left-20 top-16 -z-10 h-72 w-72 rounded-full bg-academy-gold/20 blur-3xl" />
      <div className="page-container">
        <div className="grid items-center gap-8 lg:grid-cols-[1.08fr_.92fr] lg:gap-12">
          <div>
            <span className="inline-flex items-center gap-2 rounded-full border border-orange-300/35 bg-orange-400/10 px-4 py-2 text-[.68rem] font-black uppercase tracking-[.2em] text-orange-200"><span className="h-2.5 w-2.5 rounded-full bg-orange-400 shadow-[0_0_16px_rgba(251,146,60,.9)]" />SSIAP 1 · formation réglementée</span>
            <h1 className="mt-5 max-w-4xl text-4xl font-black tracking-[-.055em] sm:text-5xl lg:text-6xl xl:text-7xl">Devenez agent de <span className="text-orange-300">sécurité incendie.</span></h1>
            ...
            <div className="mt-6 flex flex-col gap-3 sm:flex-row"><CTA href={sessionHref(next)} variant="gold">Voir les prochaines dates →</CTA><CTA href="tel:0422470768" variant="outline">Parler à un conseiller</CTA></div>
    ...
    <Section id="admission" eyebrow="Admission" title={<>Les prérequis, expliqués simplement.</>} intro={<>Notre équipe vérifie chaque pièce avant l’entrée en formation pour sécuriser votre admission et éviter toute mauvaise surprise le jour de l’examen.</>} tone="paper">
      <div className="grid gap-4 md:grid-cols-2">{prerequisites.map(([title, text], index) => <article key={title} className="rounded-[1.7rem] border border-academy-line bg-white p-5 shadow-soft"><div className="flex items-start gap-4"><span className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-orange-100 font-black text-orange-700">{index + 1}</span><div><h3 className="text-xl font-black">{title}</h3><p className="mt-2 leading-7 text-academy-muted">{text}</p></div></div></article>)}</div>
      <div className="mt-8 grid gap-5 lg:grid-cols-2"><article className="rounded-[2rem] border border-academy-line bg-academy-bg p-6 lg:p-8"><p className="text-xs font-black uppercase tracking-[.2em] text-orange-700">Formule essentielle</p><h3 className="mt-3 text-3xl font-black">SSIAP 1</h3><p className="mt-3 leading-7 text-academy-muted">Pour les candidats dont la qualification de secourisme est déjà en cours de validité.</p><p className="mt-6 text-4xl font-black">980 €</p></article><article className="rounded-[2rem] bg-[#0D1725] p-6 text-white shadow-card lg:p-8"><p className="text-xs font-black uppercase tracking-[.2em] text-orange-300">Formule complète</p><h3 className="mt-3 text-3xl font-black">SSIAP 1 + SST</h3><p className="mt-3 leading-7 text-white/65">Pour obtenir ou renouveler votre qualification de secourisme avant la formation SSIAP.</p><p className="mt-6 text-4xl font-black text-orange-300">1 230 €</p></article></div>
      <div className="mt-10"><Eyebrow>Inscription</Eyebrow><h3 className="mt-3 text-3xl font-black">Cinq étapes, un dossier maîtrisé.</h3><div className="mt-6 grid gap-3 md:grid-cols-5">{enrollment.map((item, index) => <div key={item} className="relative rounded-[1.4rem] border border-academy-line bg-white p-4"><span className="text-3xl font-black text-orange-500">0{index + 1}</span><p className="mt-6 font-black leading-6">{item}</p></div>)}</div></div>
    </Section>
    ...
    <Section id="programme" eyebrow="Programme" title={<>67 heures pour devenir opérationnel.</>} intro={<>Un programme réglementaire complet qui progresse des fondamentaux du feu jusqu’aux rondes et mises en situation professionnelles.</>}>
      <div className="space-y-3">{program.map(([number, title, hours, text], index) => <details key={number} open={index === 0} className="group rounded-[1.6rem] border border-academy-line bg-[#FFFDF8] p-5 shadow-soft"><summary className="flex cursor-pointer list-none items-center gap-4"><span className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-[#0D1725] font-black text-orange-300">{number}</span><span className="min-w-0 flex-1"><strong className="block text-lg font-black sm:text-xl">{title}</strong><small className="font-bold text-academy-muted">{hours}</small></span><span className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-academy-gold font-black transition group-open:rotate-45">+</span></summary><p className="ml-0 mt-5 border-t border-academy-line pt-5 leading-7 text-academy-muted sm:ml-16">{text}</p></details>)}</div>
      <div id="pedagogie" className="mt-10 grid gap-6 rounded-[2rem] bg-[#0D1725] p-6 text-white lg:grid-cols-[.8fr_1.2fr] lg:p-9"><div><Eyebrow light>Pédagogie terrain</Eyebrow><h3 className="mt-3 text-3xl font-black">On apprend en faisant.</h3><p className="mt-4 leading-7 text-white/65">Cours réglementaires, démonstrations et entraînements sont complétés par des rondes, des manipulations et des scénarios proches du terrain.</p></div><div className="grid gap-3 sm:grid-cols-2">{['Manipulation des équipements', 'Rondes avec anomalies', 'Visites applicatives', 'Mises en situation', 'Entraînement au QCM', 'Préparation à l’examen'].map((item, index) => <div key={item} className="rounded-2xl border border-white/10 bg-white/7 p-4"><span className="text-xs font-black text-orange-300">0{index + 1}</span><p className="mt-3 font-black">{item}</p></div>)}</div></div>
    </Section>
    ...
    <Section id="examen" eyebrow="Certification" title={<>Un examen officiel, préparé sans zone d’ombre.</>} intro={<>Vous connaissez les attendus, vous vous entraînez dans les conditions les plus proches de l’épreuve et vous recevez un retour précis sur votre progression.</>} tone="dark">
      <div className="grid gap-4 lg:grid-cols-3">{examSteps.map(([number, title, text], index) => <article key={number} className={`rounded-[1.8rem] border p-6 ${index === 1 ? 'border-orange-300/35 bg-orange-400/10' : 'border-white/10 bg-white/6'}`}><span className="text-5xl font-black text-orange-300">{number}</span><h3 className="mt-8 text-2xl font-black">{title}</h3><p className="mt-3 leading-7 text-white/65">{text}</p></article>)}</div>
    ...
    <Section id="debouches" eyebrow="Après le SSIAP 1" title={<>Travaillez, évoluez, maintenez vos compétences.</>} intro={<>Le SSIAP 1 ouvre l’accès aux fonctions d’agent de sécurité incendie. L’expérience permet ensuite d’envisager le SSIAP 2 et la responsabilité d’une équipe.</>}>
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-[1.15fr_.85fr]"><article className="rounded-[2rem] border border-academy-line bg-white p-6"><Eyebrow>Où exercer ?</Eyebrow><div className="mt-5 flex flex-wrap gap-2">{workplaces.map(item => <span key={item} className="rounded-full border border-academy-line bg-academy-bg px-4 py-2 text-sm font-bold">{item}</span>)}</div></article><article className="rounded-[2rem] border border-orange-200 bg-orange-50 p-6"><Eyebrow>Maintien des acquis</Eyebrow><h3 className="mt-3 text-2xl font-black">Recyclage tous les 3 ans</h3><p className="mt-3 leading-7 text-academy-muted">Le secourisme doit rester à jour. Une remise à niveau peut être nécessaire selon la date d’expiration et l’activité exercée.</p></article></div>
      <div className="mt-8 grid gap-4 rounded-[2rem] bg-[#EFE7D9] p-6 md:grid-cols-3 lg:p-8">{[['Centre agréé', 'SSIAP n°8323'], ['Cadre officiel', 'Arrêté n°26/099 du 29 mai 2026'], ['Formation suivie', '12 stagiaires maximum']].map(([title, text]) => <div key={title} className="rounded-2xl bg-white p-5 text-center"><p className="text-sm font-black uppercase tracking-[.16em] text-orange-700">{title}</p><p className="mt-3 text-xl font-black">{text}</p></div>)}</div>
    </Section>

    <div id="faq-ssiap"><PremiumFAQSection badge="FAQ SSIAP 1" title="Vos questions sur la formation SSIAP 1" description="Prérequis, examen, financement, validité et évolution : retrouvez les réponses essentielles avant votre inscription." items={faq} contactHref={contactHref('question SSIAP 1')} /></div>

    <section className="bg-academy-bg px-4 pb-20 pt-8"><div className="page-container overflow-hidden rounded-[2.4rem] border border-academy-line bg-[#FFFDF8] shadow-card"><div className="grid items-center lg:grid-cols-[1.05fr_.95fr]"><div className="p-7 sm:p-9 lg:p-12"><Eyebrow>Une question sur votre projet ?</Eyebrow><h2 className="mt-4 text-4xl font-black tracking-[-.05em] sm:text-5xl">Faites le premier pas vers votre futur métier.</h2><p className="mt-5 max-w-xl text-lg leading-8 text-academy-muted">Cassandre vous aide à vérifier vos prérequis, choisir votre session et trouver la solution de financement adaptée.</p><div className="mt-6 flex flex-col gap-3 sm:flex-row"><CTA href={contactHref('rendez-vous avec Cassandre')} variant="dark">Réserver un rendez-vous →</CTA><CTA href="tel:0422470768" variant="light">Appeler Cassandre</CTA></div></div><div className="relative min-h-[370px] bg-[radial-gradient(circle_at_55%_30%,rgba(238,184,47,.35),transparent_32%),linear-gradient(140deg,#EEE4D4,#FFFDF8)] p-7"><div className="absolute inset-x-7 bottom-7 rounded-[1.8rem] border border-white/75 bg-white/88 p-5 shadow-soft backdrop-blur"><div className="flex items-center gap-4"><Image src="/images/cassandre-memoji.png" width={88} height={88} alt="Cassandre, responsable commerciale Intégrale Academy" className="h-20 w-20 rounded-2xl object-cover" /><div><p className="text-xs font-black uppercase tracking-[.18em] text-orange-700">Votre conseillère</p><p className="mt-1 text-2xl font-black">Cassandre</p><p className="text-sm font-semibold text-academy-muted">Responsable commerciale</p></div></div><a href="tel:0422470768" className="mt-4 block rounded-full bg-academy-gold px-5 py-3 text-center font-black text-academy-gold-text">04 22 47 07 68</a></div></div></div></div></section>

    <div className="fixed inset-x-0 bottom-0 z-40 border-t border-academy-line bg-[#FFFDF8]/96 p-3 shadow-[0_-12px_40px_rgba(0,0,0,.12)] backdrop-blur lg:hidden"><div className="mx-auto flex max-w-lg gap-2"><CTA href="tel:0422470768" variant="light" className="min-w-0 flex-1 px-3">Appeler</CTA><CTA href={sessionHref(next)} variant="gold" className="min-w-0 flex-[1.4] px-3">Réserver ma place</CTA></div></div>
  </main>;
}
