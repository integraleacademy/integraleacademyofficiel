...
function SocialLinks({ compact = false }: { compact?: boolean }){
  return <div className={compact ? 'mt-4 grid gap-2' : 'mt-5 grid gap-3'}>
    {contact.socialLinks.map((social) => <a key={social.href} href={social.href} target="_blank" rel="noopener noreferrer" className="group flex items-center justify-between gap-3 rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-stone-200 transition hover:border-academy-gold/60 hover:bg-academy-gold/10 hover:text-white"><span><span className="font-black text-white group-hover:text-academy-gold">{social.label}</span><span className="block text-xs text-stone-400">{social.handle}</span></span><span className="text-academy-gold" aria-hidden="true">↗</span></a>)}
  </div>
}
