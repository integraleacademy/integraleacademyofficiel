...
export function FullWidthBand({eyebrow,title,children,actions,tone='dark'}:{eyebrow?:string;title:string;children:React.ReactNode;actions?:React.ReactNode;tone?:'dark'|'gold'|'light'}){
  ...
  return <section className={`relative isolate overflow-hidden px-4 py-12 md:py-16 ${shell} ${border}`}>
    ...
    <div className="mx-auto grid max-w-7xl gap-6 md:grid-cols-[1fr_auto] md:items-center"><div className="max-w-3xl">{eyebrow&&<p className={`text-xs font-black uppercase tracking-[.22em] ${isGold?'text-stone-700':isLight?'text-yellow-700':'text-academy-gold'}`}>{eyebrow}</p>}<h2 className="mt-3 text-3xl font-black tracking-tight md:text-5xl">{title}</h2><p className={`mt-4 text-base leading-8 md:text-lg ${muted}`}>{children}</p></div>{actions&&<div className="flex flex-wrap gap-3 md:justify-end">{actions}</div>}</div>
  </section>
}
...
export function ContactBlock(){return <section className="px-0 py-4"><div className="mx-auto max-w-6xl rounded-3xl bg-white p-6 shadow-sm ring-1 ring-academy-line md:p-8"><div className="grid gap-6 md:grid-cols-[1fr_auto] md:items-center"><div><p className="inline-flex items-center gap-2 rounded-full bg-academy-green/10 px-3 py-1.5 text-xs font-black uppercase tracking-[.16em] text-green-700"><span className="h-2 w-2 rounded-full bg-academy-green"/>Conseiller disponible</p><h2 className="mt-4 text-2xl font-black tracking-tight md:text-4xl">Prêt à démarrer votre formation&nbsp;?</h2><p className="mt-3 max-w-2xl text-sm leading-7 text-stone-600 md:text-base">Parlez-nous de votre projet : sécurité, VTC ou BTS. Nous vous aidons à choisir le bon parcours, vérifier le financement et préparer votre inscription.</p><p className="mt-3 text-sm font-semibold text-stone-500">Sans engagement · Réponse rapide · Accompagnement personnalisé</p></div><div className="flex flex-col gap-3 sm:flex-row md:flex-col"><Link href="/contact" className="inline-flex items-center justify-center rounded-full bg-academy-ink px-6 py-3 text-sm font-black text-white transition hover:-translate-y-0.5 hover:bg-black">Demander des informations</Link><Link href="tel:0422470768" className="inline-flex items-center justify-center rounded-full bg-academy-gold px-6 py-3 text-sm font-black text-academy-ink transition hover:-translate-y-0.5 hover:brightness-95">Appeler un conseiller</Link></div></div></div></section>}
...
export function CTASection(){return <ContactBlock/>}
