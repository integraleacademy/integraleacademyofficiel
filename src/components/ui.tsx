...
export function Header(){
  ...
  return <header className="sticky top-0 z-50 border-b border-white/60 bg-academy-surface/80 shadow-[0_18px_50px_rgba(17,17,17,.06)] backdrop-blur-xl"><div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-3"><Link href="/" aria-label="Retour à l’accueil Intégrale Academy" className="group flex items-center gap-3 rounded-full pr-2 text-lg font-black focus:outline-none focus:ring-2 focus:ring-academy-gold"><span className="grid h-12 w-12 place-items-center rounded-[1.25rem] bg-gradient-to-br from-academy-gold to-academy-gold text-academy-gold-text shadow-gold transition group-hover:rotate-3" aria-hidden="true">IA</span><span className="leading-tight">Intégrale<br className="sm:hidden"/><span className="text-academy-gold-strong"> Academy</span></span></Link><nav className="hidden items-center gap-1 text-sm font-bold lg:flex" aria-label="Navigation principale">{nav.map(([n,h])=>typeof h!=='string'?<div key={n} className="group relative"><span className="invisible absolute left-1/2 top-full h-5 w-[30rem] -translate-x-1/2" aria-hidden="true"/><button type="button" className="inline-flex items-center gap-1 rounded-full px-3 py-2 font-bold text-academy-muted transition hover:bg-academy-gold/10 hover:text-academy-gold-strong focus:bg-academy-gold/10 focus:text-academy-gold-strong focus:outline-none focus:ring-2 focus:ring-academy-gold" aria-haspopup="true"><span>{n}</span><span className="text-academy-gold-strong transition group-hover:rotate-180" aria-hidden="true">⌄</span></button><div className="invisible absolute left-1/2 top-full z-50 mt-3 w-[30rem] -translate-x-1/2 translate-y-3 overflow-hidden rounded-[2rem] border border-academy-gold/25 bg-academy-surface/95 p-3 opacity-0 shadow-[0_28px_80px_rgba(55,43,18,.18)] ring-1 ring-white/70 backdrop-blur-2xl transition duration-200 group-hover:visible group-hover:translate-y-0 group-hover:opacity-100 group-focus-within:visible group-focus-within:translate-y-0 group-focus-within:opacity-100"><div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(244,196,90,.28),transparent_38%),linear-gradient(135deg,rgba(255,255,255,.92),rgba(255,248,232,.72))]" aria-hidden="true"/><div className="relative rounded-[1.5rem] border border-white/70 bg-white/55 p-3"><p className="px-3 pb-2 text-[0.68rem] font-black uppercase tracking-[.22em] text-academy-gold-strong">Choisir une famille</p><div className="grid gap-2">{h.map(([label,href,description,icon])=><Link key={href} href={href} className="group/item flex items-center gap-4 rounded-[1.25rem] p-3 text-academy-ink transition hover:-translate-y-0.5 hover:bg-white hover:shadow-card focus:-translate-y-0.5 focus:bg-white focus:shadow-card focus:outline-none"><span className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-academy-gold/18 text-xl shadow-inner transition group-hover/item:scale-105" aria-hidden="true">{icon}</span><span className="min-w-0"><span className="block font-black">{label}</span><span className="mt-1 block text-xs font-semibold leading-5 text-academy-muted">{description}</span></span><span className="ml-auto text-lg text-academy-gold-strong transition group-hover/item:translate-x-1" aria-hidden="true">→</span></Link>)}</div></div></div></div>:<Link key={h} href={h} className="rounded-full px-3 py-2 text-academy-muted transition hover:text-academy-gold-strong focus:text-academy-gold-strong">{n}</Link>)}</nav><div className="hidden items-center gap-2 xl:flex"><ThemeToggle/><Button href="tel:0422470768" variant="ghost">Appeler</Button><AppointmentButton/></div><div className="flex items-center gap-2 lg:hidden"><ThemeToggle/><MobileHeaderMenu nav={nav} appointmentFormUrl={appointmentFormUrl}/></div></div></header>
}
