...
const menuSecurityFormations = securityFormations.filter(formation => !['/formations-securite/desp-initial', '/formations-securite/desp-vae'].includes(formation.slug));
  ...
  return <div className="rounded-[1.25rem] bg-academy-surface p-4 text-academy-gold-text shadow-gold sm:p-5">
    ...
    <div className="ai-chat-preview mt-5 grid min-h-[22rem] pb-8 sm:min-h-[20rem]" aria-label="Aperçu animé d’une conversation avec l’assistant IA">
      {aiChatConversations.map((conversation, index) => <div key={conversation.user} className={`ai-chat-sequence ai-chat-sequence-${index + 1} col-start-1 row-start-1 space-y-3 pb-4`}>
        <div className={`${bubbleBase} ai-chat-message ai-chat-delay-1 bg-academy-bg text-academy-muted`}>{conversation.intro}</div>
...
export function Badge({children,tone='gold'}:{children:React.ReactNode;tone?:'gold'|'green'|'neutral'}){const c=tone==='green'?'border border-green-700/20 bg-academy-green/15 text-green-800 dark:text-green-300':tone==='gold'?'border border-academy-gold/45 bg-academy-gold-soft/80 text-academy-gold-strong shadow-sm dark:bg-academy-gold/25 dark:text-academy-gold':'bg-academy-surface text-academy-muted border border-academy-line';return <span className={`inline-flex rounded-full px-3 py-1 text-xs font-black ${c}`}>{children}</span>}
export function Button({href,children,variant='primary'}:{href:string;children:React.ReactNode;variant?:'primary'|'secondary'|'ghost'}){const c=variant==='primary'?'bg-academy-ink text-academy-bg hover:brightness-110 dark:bg-academy-elevated dark:text-academy-ink':variant==='secondary'?'bg-academy-gold text-academy-gold-text shadow-gold hover:brightness-105':'bg-academy-surface text-academy-ink ring-1 ring-academy-line hover:bg-academy-soft';return <Link className={`inline-flex items-center justify-center rounded-full px-5 py-3 text-sm font-bold transition hover:-translate-y-0.5 ${c}`} href={href}>{children}</Link>}
export function SectionTitle({eyebrow,title,children}:{eyebrow?:string;title:string;children?:React.ReactNode}){return <div className="mx-auto mb-10 max-w-3xl text-center reveal">{eyebrow&&<Badge>{eyebrow}</Badge>}<h2 className="mt-4 text-3xl font-black tracking-tight md:text-5xl">{title}</h2>{children&&<p className="mt-4 text-lg text-academy-muted">{children}</p>}</div>}
export function Hero({badge,title,subtitle,actions,visual}:{badge?:string;title:string;subtitle:string;actions?:React.ReactNode;visual?:React.ReactNode}){return <section className="grid-soft gold-glow overflow-hidden px-4 py-16 md:py-24"><div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[1.15fr_.85fr] lg:items-center"><div className="reveal">{badge&&<Badge>{badge}</Badge>}<h1 className="mt-5 text-4xl font-black tracking-tight text-academy-ink md:text-6xl">{title}</h1><p className="mt-6 max-w-2xl text-lg font-medium leading-8 text-academy-muted">{subtitle}</p>{actions&&<div className="mt-8 flex flex-wrap gap-3">{actions}</div>}</div>{visual ?? <div className="relative rounded-[2rem] bg-academy-surface p-6 shadow-card ring-1 ring-academy-line backdrop-blur reveal"><div className="rounded-[1.5rem] bg-academy-bg p-6"><p className="text-sm font-bold text-academy-muted">Centres & modalités</p><div className="mt-5 grid gap-3">{contact.locations.map(l=><div key={l.name} className="rounded-2xl bg-academy-surface p-4"><b>{l.name}</b><p className="text-sm text-academy-muted">{l.address}</p></div>)}</div></div></div>}</div></section>}
...
export function Header(){
  ...
  const dropdownLinkClass="group/item rounded-2xl border border-transparent p-3 transition hover:border-academy-gold/50 hover:bg-academy-gold-soft/60 focus:border-academy-gold/50 focus:bg-academy-gold-soft/60";
  return <header className="sticky top-0 z-50 border-b border-white/60 bg-academy-surface/80 shadow-[0_18px_50px_rgba(17,17,17,.06)] backdrop-blur-xl"><div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-3"><Link href="/" className="group flex items-center gap-3 rounded-full pr-2 text-lg font-black focus:outline-none focus:ring-2 focus:ring-academy-gold"><span className="grid h-12 w-12 place-items-center rounded-[1.25rem] bg-gradient-to-br from-academy-gold to-academy-gold text-academy-gold-text shadow-gold transition group-hover:rotate-3" aria-hidden="true">IA</span><span className="leading-tight">Intégrale<br className="sm:hidden"/><span className="text-academy-gold-strong"> Academy</span></span></Link><nav className="hidden items-center gap-1 text-sm font-bold lg:flex" aria-label="Navigation principale"><div className="group relative"><Link href="/formations-securite" className="inline-flex items-center gap-1 rounded-full px-3 py-2 text-academy-muted transition hover:text-academy-gold-strong focus:text-academy-gold-strong">Formations sécurité <span aria-hidden="true">⌄</span></Link><div className="pointer-events-none absolute left-1/2 top-full w-[34rem] -translate-x-1/2 pt-4 opacity-0 transition duration-200 group-hover:pointer-events-auto group-hover:opacity-100 group-focus-within:pointer-events-auto group-focus-within:opacity-100"><div className="overflow-hidden rounded-[2rem] bg-academy-surface p-3 shadow-[0_30px_80px_rgba(17,17,17,.14)] ring-1 ring-academy-line"><div className="rounded-[1.5rem] bg-gradient-to-br from-stone-950 to-stone-800 p-5 text-white"><p className="text-xs font-black uppercase tracking-[.2em] text-academy-gold">Sécurité privée & incendie</p><p className="mt-2 text-sm text-stone-200">Trouvez rapidement la certification adaptée à votre projet.</p></div><div className="mt-2 grid grid-cols-2 gap-1">{menuSecurityFormations.map(f=><Link key={f.slug} href={f.slug} className={dropdownLinkClass}><span className="block text-sm font-black text-academy-ink transition group-hover/item:text-academy-gold-strong">{f.title}</span><span className="mt-1 line-clamp-2 block text-xs leading-5 text-academy-muted">{f.duration}</span></Link>)}</div></div></div></div><div className="group relative"><Link href="/bts" className="inline-flex items-center gap-1 rounded-full px-3 py-2 text-academy-muted transition hover:text-academy-gold-strong focus:text-academy-gold-strong">BTS <span aria-hidden="true">⌄</span></Link><div className="pointer-events-none absolute left-1/2 top-full w-[38rem] -translate-x-1/2 pt-4 opacity-0 transition duration-200 group-hover:pointer-events-auto group-hover:opacity-100 group-focus-within:pointer-events-auto group-focus-within:opacity-100"><div className="overflow-hidden rounded-[2rem] bg-academy-surface p-3 shadow-[0_30px_80px_rgba(17,17,17,.14)] ring-1 ring-academy-line"><div className="rounded-[1.5rem] bg-gradient-to-br from-academy-gold to-academy-gold-soft p-5"><p className="text-xs font-black uppercase tracking-[.2em] text-academy-muted">Alternance & diplômes d’État</p><p className="mt-2 text-sm font-semibold text-stone-800">Commerce, sécurité, immobilier, international : choisissez votre parcours.</p></div><div className="mt-2 grid grid-cols-2 gap-1">{btsFormations.map(f=><Link key={f.slug} href={f.slug} className={dropdownLinkClass}><span className="block text-sm font-black text-academy-ink transition group-hover/item:text-academy-gold-strong">{f.title}</span><span className="mt-1 block text-xs leading-5 text-academy-muted">{f.tags.join(' · ')}</span></Link>)}</div></div></div></div>{nav.map(([n,h])=><Link key={h} href={h} className="rounded-full px-3 py-2 text-academy-muted transition hover:text-academy-gold-strong focus:text-academy-gold-strong">{n}</Link>)}</nav><div className="hidden items-center gap-2 xl:flex"><ThemeToggle/><Button href="tel:0422470768" variant="ghost">Appeler</Button><AppointmentButton/></div><div className="flex items-center gap-2 lg:hidden"><ThemeToggle/><details className="relative"><summary className="list-none rounded-full border border-academy-line bg-academy-surface px-4 py-2 text-sm font-bold shadow-soft cursor-pointer">Menu</summary><div className="absolute right-0 mt-3 max-h-[80vh] w-[min(22rem,calc(100vw-2rem))] overflow-y-auto rounded-3xl bg-academy-surface p-4 shadow-soft ring-1 ring-academy-line"><nav className="grid gap-2" aria-label="Navigation mobile"><Link href="/" className="rounded-2xl px-4 py-3 font-semibold hover:text-academy-gold-strong">Accueil</Link><details className="rounded-2xl bg-academy-bg p-3"><summary className="cursor-pointer font-black">Formations sécurité</summary><div className="mt-2 grid gap-1">{menuSecurityFormations.map(f=><Link key={f.slug} href={f.slug} className="rounded-xl px-3 py-2 text-sm font-semibold text-academy-muted hover:text-academy-gold-strong">{f.title}</Link>)}</div></details><details className="rounded-2xl bg-academy-bg p-3"><summary className="cursor-pointer font-black">BTS</summary><div className="mt-2 grid gap-1">{btsFormations.map(f=><Link key={f.slug} href={f.slug} className="rounded-xl px-3 py-2 text-sm font-semibold text-academy-muted hover:text-academy-gold-strong">{f.title}</Link>)}</div></details>{nav.slice(1).map(([n,h])=><Link key={h} href={h} className="rounded-2xl px-4 py-3 font-semibold hover:text-academy-gold-strong">{n}</Link>)}</nav><div className="mt-3 grid gap-2"><Button href="tel:0422470768" variant="ghost">Appeler</Button><Button href="/contact">Demander des informations</Button><AppointmentButton/></div></div></details></div></div></header>
}
...
export function FAQ({items}:{items:{q:string;a:string}[]}){return <div className="mx-auto max-w-3xl space-y-3">{items.map(i=><details key={i.q} className="rounded-2xl bg-academy-surface p-5 ring-1 ring-academy-line"><summary className="cursor-pointer font-bold">{i.q}</summary><p className="mt-3 text-sm text-academy-muted">{i.a}</p></details>)}</div>}
