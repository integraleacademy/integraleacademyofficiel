...
export function Badge({children,tone='gold'}:{children:React.ReactNode;tone?:UiAccent|'neutral'}){
  const styles: Record<UiAccent|'neutral', string> = {
    gold: 'bg-academy-gold/25 text-stone-900',
    ...
    neutral: 'border border-academy-line bg-white text-stone-700',
  };
  return <span className={`inline-flex rounded-full px-3 py-1 text-xs font-semibold ${styles[tone]}`}>{children}</span>;
}

type ButtonVariant = 'primary' | 'secondary' | 'ghost' | 'bts' | Exclude<UiAccent, 'gold'>;

export function Button({href,children,variant='primary',className=''}:{href:string;children:React.ReactNode;variant?:ButtonVariant;className?:string}){
  const styles: Record<ButtonVariant, string> = {
    primary: 'bg-academy-ink text-white hover:bg-black focus:ring-academy-gold/35',
    secondary: 'bg-academy-gold text-academy-ink hover:brightness-95 focus:ring-academy-gold/35',
    ghost: 'bg-white text-academy-ink ring-1 ring-academy-line hover:bg-stone-50 focus:ring-stone-300/55',
    blue: 'bg-blue-600 text-white hover:bg-blue-700 focus:ring-blue-300/55',
    ...
    bts: 'bg-bts-700 text-white hover:bg-bts-800 focus:ring-bts-300/55',
  };
  return <Link className={`inline-flex max-w-full items-center justify-center whitespace-normal rounded-full px-5 py-3 text-center text-sm font-bold leading-tight transition hover:-translate-y-0.5 focus:outline-none focus:ring-4 ${styles[variant]} ${className}`} href={href}>{children}</Link>;
}
...
const artDirectionCopy: Record<ArtDirectionWorld, { label: string; title: string; chips: string[]; stats: [string, string][]; glyphs: string[] }> = {
  ...
  bts: {
    ...
    stats: [['2 ans', 'alternance'], ['6', 'spécialités'], ['OPCO', 'prise en charge']],
    glyphs: ['◇', '▱', '✦'],
  },
  vtc: {
...
export function Hero({badge,title,subtitle,actions,visual,theme='gold'}:{badge?:string;title:React.ReactNode;subtitle:string;actions?:React.ReactNode;visual?:React.ReactNode;theme?:UiAccent}){
  const glows: Record<UiAccent, string> = {
    ...
    green: 'training-glow-green',
    orange: 'training-glow-orange',
...
export function Footer(){
  const footerLinks=[
    {title:'Formations',links:[['APS','/formations-securite/aps'],['SSIAP','/formations-securite/ssiap'],['A3P','/formations-securite/a3p-apr'],['BTS en alternance','/bts'],['Chauffeur VTC','/vtc']]},
...
const premiumFaqThemes: Record<UiAccent | 'bts', { shell:string; glow:string; badge:string; contact:string; icon:string; item:string; bar:string; number:string; toggle:string; check:string }> = {
  bts: { shell:'bg-gradient-to-br from-bts-50 via-white to-bts-100', glow:'bg-bts-400/20', badge:'border-bts-300/45 text-bts-700', contact:'border-bts-300/45', icon:'bg-bts-100 text-bts-700', item:'border-bts-200/75 hover:border-bts-400 open:border-bts-500', bar:'bg-bts-700', number:'bg-bts-50 text-bts-700', toggle:'bg-bts-700 text-white shadow-soft', check:'bg-bts-100 text-bts-700' },
  gold: { shell:'bg-[linear-gradient(135deg,#F7F1E6_0%,#FFFDF8_48%,#F2E6D1_100%)]', glow:'bg-academy-gold/18', badge:'border-academy-gold/30 text-academy-gold-strong', contact:'border-academy-gold/35', icon:'bg-academy-gold/20 text-academy-gold-strong', item:'border-academy-gold/20 hover:border-academy-gold/40 open:border-academy-gold/55', bar:'bg-academy-gold', number:'bg-academy-bg text-academy-gold-strong', toggle:'bg-gradient-to-br from-academy-gold to-[#F6D77F] text-academy-gold-text shadow-gold', check:'bg-academy-gold/20 text-academy-gold-strong' },
  blue: { shell:'bg-[linear-gradient(135deg,#EFF6FF_0%,#FFFFFF_48%,#DBEAFE_100%)]', glow:'bg-blue-400/18', badge:'border-blue-300/45 text-blue-700', contact:'border-blue-300/45', icon:'bg-blue-100 text-blue-700', item:'border-blue-200/75 hover:border-blue-400 open:border-blue-500', bar:'bg-blue-600', number:'bg-blue-50 text-blue-700', toggle:'bg-gradient-to-br from-blue-600 to-sky-500 text-white shadow-[0_12px_28px_rgba(37,99,235,.22)]', check:'bg-blue-100 text-blue-700' },
  green: { shell:'bg-[linear-gradient(135deg,#ECFDF5_0%,#FFFFFF_48%,#D1FAE5_100%)]', glow:'bg-emerald-400/18', badge:'border-emerald-300/45 text-emerald-700', contact:'border-emerald-300/45', icon:'bg-emerald-100 text-emerald-700', item:'border-emerald-200/75 hover:border-emerald-400 open:border-emerald-500', bar:'bg-emerald-600', number:'bg-emerald-50 text-emerald-700', toggle:'bg-gradient-to-br from-emerald-600 to-green-500 text-white shadow-[0_12px_28px_rgba(5,150,105,.22)]', check:'bg-emerald-100 text-emerald-700' },
  orange: { shell:'bg-[linear-gradient(135deg,#FFF7ED_0%,#FFFFFF_48%,#FFEDD5_100%)]', glow:'bg-orange-400/18', badge:'border-orange-300/45 text-orange-700', contact:'border-orange-300/45', icon:'bg-orange-100 text-orange-700', item:'border-orange-200/75 hover:border-orange-400 open:border-orange-500', bar:'bg-orange-600', number:'bg-orange-50 text-orange-700', toggle:'bg-gradient-to-br from-orange-600 to-amber-500 text-white shadow-[0_12px_28px_rgba(234,88,12,.22)]', check:'bg-orange-100 text-orange-700' },
  red: { shell:'bg-[linear-gradient(135deg,#FEF2F2_0%,#FFFFFF_48%,#FEE2E2_100%)]', glow:'bg-red-400/18', badge:'border-red-300/45 text-red-700', contact:'border-red-300/45', icon:'bg-red-100 text-red-700', item:'border-red-200/75 hover:border-red-400 open:border-red-500', bar:'bg-red-600', number:'bg-red-50 text-red-700', toggle:'bg-gradient-to-br from-red-600 to-rose-500 text-white shadow-[0_12px_28px_rgba(220,38,38,.22)]', check:'bg-red-100 text-red-700' },
  violet: { shell:'bg-[linear-gradient(135deg,#F5F3FF_0%,#FFFFFF_48%,#EDE9FE_100%)]', glow:'bg-violet-400/18', badge:'border-violet-300/45 text-violet-700', contact:'border-violet-300/45', icon:'bg-violet-100 text-violet-700', item:'border-violet-200/75 hover:border-violet-400 open:border-violet-500', bar:'bg-violet-600', number:'bg-violet-50 text-violet-700', toggle:'bg-gradient-to-br from-violet-600 to-purple-500 text-white shadow-[0_12px_28px_rgba(124,58,237,.22)]', check:'bg-violet-100 text-violet-700' },
};

export function FAQ({items,theme='gold'}:{items:{q:string;a:string}[];theme?:UiAccent}){const styles=premiumFaqThemes[theme];return <div className="mx-auto max-w-3xl space-y-4">{items.map(i=><details key={i.q} className={`group overflow-hidden rounded-[1.35rem] border bg-academy-surface shadow-soft transition duration-300 open:shadow-card ${styles.item}`}><summary className="flex cursor-pointer list-none items-center justify-between gap-4 p-5 text-base font-black leading-6 transition hover:bg-academy-bg/60 sm:p-6"><span>{i.q}</span><span className={`grid h-9 w-9 shrink-0 place-items-center rounded-full transition duration-300 group-open:rotate-45 ${styles.toggle}`} aria-hidden="true">+</span></summary><div className="px-5 pb-5 sm:px-6 sm:pb-6"><p className="border-t border-academy-line pt-4 text-sm font-medium leading-7 text-academy-muted sm:text-base">{i.a}</p></div></details>)}</div>}

export function PremiumFAQSection({badge,title,description,items,contactHref='/contact',contactLabel='Je souhaite en savoir plus',callHref='tel:0422470768',callLabel='Être rappelé',theme='gold'}:{badge:string;title:React.ReactNode;description:React.ReactNode;items:{q:string;a:string}[];contactHref?:string;contactLabel?:string;callHref?:string;callLabel?:string;theme?:UiAccent|'bts'}){const styles=premiumFaqThemes[theme];const callVariant:ButtonVariant=theme==='gold'?'primary':theme;return <section className={`relative isolate overflow-hidden py-14 lg:py-16 ${styles.shell}`}><div className={`absolute left-0 top-0 -z-10 h-72 w-72 -translate-x-1/3 rounded-full blur-3xl ${styles.glow}`} aria-hidden="true"/><div className="absolute bottom-0 right-0 -z-10 h-96 w-96 translate-x-1/4 rounded-full bg-white/80 blur-3xl" aria-hidden="true"/><div className="page-container grid gap-7 lg:grid-cols-[0.78fr_1.22fr] lg:gap-10"><div className="reveal lg:sticky lg:top-28 lg:self-start"><span className={`inline-flex rounded-full border bg-white/80 px-4 py-2 text-xs font-black uppercase tracking-[.24em] shadow-soft backdrop-blur ${styles.badge}`}>{badge}</span><h2 className="mt-4 text-3xl font-black tracking-tight text-academy-ink sm:text-4xl lg:text-[2.7rem] lg:leading-[1.05]">{title}</h2><p className="mt-4 max-w-xl text-base font-semibold leading-7 text-academy-muted">{description}</p><div className={`mt-6 overflow-hidden rounded-[1.5rem] border bg-white/85 p-5 shadow-[0_20px_55px_rgba(54,43,24,.10)] backdrop-blur ${styles.contact}`}><div className="flex items-start gap-3"><span className={`grid h-10 w-10 shrink-0 place-items-center rounded-2xl text-lg ${styles.icon}`} aria-hidden="true">☎️</span><div><h3 className="text-base font-black text-academy-ink">Besoin d’un renseignement ?</h3><p className="mt-1 text-sm font-semibold leading-6 text-academy-muted">Notre équipe peut vous accompagner avant votre inscription.</p></div></div><div className="mt-4 flex flex-wrap gap-2"><Button href={callHref} variant={callVariant}>{callLabel}</Button><Button href={contactHref} variant="ghost">{contactLabel}</Button></div></div></div><div className="grid gap-3 md:grid-cols-2 lg:gap-4">{items.map((item,index)=><details key={item.q} className={`group reveal overflow-hidden rounded-[1.35rem] border bg-white/92 shadow-[0_14px_42px_rgba(54,43,24,.075)] transition duration-300 hover:-translate-y-0.5 hover:shadow-[0_20px_60px_rgba(54,43,24,.11)] open:shadow-[0_24px_70px_rgba(54,43,24,.14)] md:open:col-span-2 ${styles.item}`}><summary className="relative flex cursor-pointer list-none items-center justify-between gap-3 p-4 text-[0.95rem] font-black leading-6 text-academy-ink transition sm:px-5 sm:py-4 sm:text-base"><span className={`absolute inset-y-4 left-0 w-1 rounded-r-full opacity-0 transition group-open:opacity-100 ${styles.bar}`} aria-hidden="true"/><span className="flex items-start gap-3"><span className={`hidden min-w-8 rounded-full px-2.5 py-1 text-center text-[0.68rem] font-black sm:inline-block ${styles.number}`}>{String(index+1).padStart(2,'0')}</span><span>{item.q}</span></span><span className={`relative grid h-9 w-9 shrink-0 place-items-center rounded-full text-lg font-black transition duration-300 group-open:rotate-180 ${styles.toggle}`} aria-hidden="true"><span className="transition group-open:opacity-0">+</span><span className="absolute opacity-0 transition group-open:opacity-100">−</span></span></summary><div className="grid grid-rows-[0fr] transition-[grid-template-rows] duration-500 ease-out group-open:grid-rows-[1fr]"><div className="overflow-hidden"><div className="px-4 pb-5 sm:px-5"><p className="border-t border-academy-line/80 pt-3 text-[0.95rem] font-medium leading-7 text-academy-muted sm:text-base sm:leading-8">{item.a}</p></div></div></div></details>)}</div></div></section>}
...
export function ConversionStrip({theme='gold'}:{theme?:UiAccent}){const styles=premiumFaqThemes[theme];const actionVariant:ButtonVariant=theme==='gold'?'primary':theme;const callbackVariant:ButtonVariant=theme==='gold'?'secondary':theme;return <section className="page-container py-8"><div className="grid gap-4 rounded-[2rem] bg-academy-surface p-5 shadow-soft ring-1 ring-academy-line md:grid-cols-3"><div><b className="text-academy-ink">Un doute sur la bonne formation ?</b><p className="mt-1 text-sm text-academy-muted">Un conseiller vérifie avec vous les prérequis, lieux, dates et financements.</p></div><div className="flex items-center gap-3"><span className={`grid h-11 w-11 place-items-center rounded-2xl font-black ${styles.check}`}>✓</span><p className="text-sm font-semibold text-academy-muted">Réponse orientée inscription, sans engagement.</p></div><div className="flex flex-wrap items-center justify-start gap-2 md:justify-end"><Button href="/contact" variant={actionVariant}>Demander des informations</Button><Button href="tel:0422470768" variant={callbackVariant}>Être rappelé</Button></div></div></section>}
