...
const contactHref = (subject = 'BTS NDRC') =>
...
const program = [
  ...
  ['02', 'Langue vivante étrangère', 'Communiquer dans un contexte professionnel et commercial avec une clientèle variée.'],
  ['03', 'Culture économique, juridique et managériale', 'Comprendre l’environnement des organisations et analyser leurs décisions économiques, juridiques et managériales.'],
  ['04', 'Relation client et négociation-vente', 'Cibler, prospecter, négocier, accompagner le client, organiser un événement et exploiter l’information commerciale.'],
  ['05', 'Relation client à distance et digitalisation', 'Maîtriser la relation omnicanale, animer la relation client digitale et développer le e-commerce.'],
  ['06', 'Relation client et animation de réseaux', 'Promouvoir une offre chez des distributeurs et développer des réseaux de partenaires ou de vente directe.'],
  ['07', 'Atelier de professionnalisation', 'Préparer les situations professionnelles, les dossiers, les outils numériques et les épreuves du BTS.'],
];
...
const faq = [
  {
    ...
    a: 'Oui. Le BTS NDRC est accessible aux titulaires d’un baccalauréat ou d’un titre équivalent, sous réserve de l’étude du dossier et de la cohérence du projet d’alternance.',
  },
  {
    q: 'Quelle différence entre le BTS NDRC et le BTS MCO ?',
    a: 'Le BTS NDRC est centré sur la prospection, la négociation, la relation client omnicanale, le digital et l’animation de réseaux. Le BTS MCO s’oriente davantage vers la gestion et le management d’une unité commerciale.',
  },
  {
    q: 'Faut-il déjà savoir vendre ?',
    a: 'Non. Il faut surtout aimer le contact, avoir envie de progresser à l’oral, être curieux et persévérant. La formation vous apprend à prospecter, argumenter, négocier et utiliser les outils numériques du commerce.',
  },
  {
  ...
  },
  {
    ...
    a: 'Oui. Le BTS Négociation et Digitalisation de la Relation Client est un diplôme national de niveau 5 délivré par le ministère de l’Enseignement supérieur et de la Recherche et enregistré au RNCP sous le numéro 38368.',
  },
  {
    q: 'Quels métiers peut-on exercer après le BTS NDRC ?',
    a: 'Le BTS prépare notamment aux métiers de commercial terrain, conseiller commercial, chargé de clientèle, négociateur, technico-commercial, conseiller client à distance, commercial e-commerce ou animateur de réseau.',
  },
  {
...
function HeroRoadmap() {
  const steps = [
    ['01', 'Prospect', 'Premier contact', '+12'],
    ['02', 'Rendez-vous', 'Besoin identifié', '+7'],
    ['03', 'Client', 'Vente conclue', '+4'],
    ['04', 'Fidélisation', 'Relation durable', '↗'],
  ];
  return (
    <aside className="relative rounded-[2rem] border border-white/60 bg-[#FFFDF8] p-5 text-academy-ink shadow-[0_34px_100px_rgba(0,0,0,.34)] sm:p-6">
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          <Eyebrow>Votre futur terrain de jeu</Eyebrow>
          <h2 className="mt-2 text-2xl font-black">Du contact au client</h2>
        </div>
        <span className="rounded-full bg-violet-50 px-3 py-1.5 text-[.6rem] font-black uppercase tracking-[.12em] text-violet-800 ring-1 ring-violet-200">
          CRM en direct
        </span>
      </div>
      <div className="mt-5 rounded-[1.5rem] border border-[#DDD3EB] bg-[#F5F0FB] p-4">
        {steps.map(([number, title, detail, status], index) => (
          <div key={number} className={`grid grid-cols-[2.3rem_1fr_auto] items-center gap-3 py-3 ${index ? 'border-t border-[#DDD3EB]' : ''}`}>
            <span className="grid h-9 w-9 place-items-center rounded-xl bg-[#0D1725] text-[.65rem] font-black text-violet-300">
              {number}
            </span>
            <div>
            ...
            <span className="rounded-full bg-white px-2.5 py-1 text-[.6rem] font-black text-violet-700 ring-1 ring-violet-200">{status}</span>
          </div>
        ))}
      </div>
      <div className="mt-3 grid grid-cols-3 gap-2">
        {([['◉', 'Terrain'], ['#', 'Réseaux'], ['⌁', 'Visio']] as const).map(([icon, label]) => (
          <div key={label} className="rounded-2xl border border-academy-line bg-white p-3 text-center">
            <p className="text-lg font-black text-violet-600">{icon}</p>
            <p className="mt-1 text-[.62rem] font-black uppercase tracking-[.1em]">{label}</p>
          </div>
        ))}
      </div>
    </aside>
  ...
  return (
    <main className="relative overflow-x-clip pb-24 lg:pb-0">
        ...
        dangerouslySetInnerHTML={{
          __html: serializeCourseJsonLd({
            ...
            '@graph': [
              {
                '@type': 'Course',
                name: 'BTS Négociation et Digitalisation de la Relation Client (NDRC)',
                description: 'BTS NDRC en alternance, en présentiel à Puget-sur-Argens ou à distance en visioconférence.',
                educationalCredentialAwarded: 'BTS - diplôme national de niveau 5 - RNCP 38368',
                provider: {
              ...
              },
              {
                ...
                itemListElement: [
                  ...
                  { '@type': 'ListItem', position: 3, name: 'BTS NDRC', item: '/bts/ndrc' },
                ],
              },
            ],
          }, "/bts/ndrc"),
      ...
      <section className="relative isolate overflow-hidden bg-[#0A1725] px-4 pb-8 pt-10 text-white sm:pt-14 lg:pt-16">
        <div className="absolute inset-0 -z-10 bg-[radial-gradient(circle_at_12%_76%,rgba(139,92,246,.31),transparent_29%),radial-gradient(circle_at_87%_12%,rgba(244,166,33,.22),transparent_28%),linear-gradient(135deg,#07111E_0%,#0E2035_62%,#111922_100%)]" />
        ...
        <div className="page-container">
          <div className="grid items-center gap-9 lg:grid-cols-[1.08fr_.92fr] lg:gap-12">
            <div>
              <span className="inline-flex items-center gap-2 rounded-full border border-violet-300/35 bg-white/8 px-4 py-2 text-[.66rem] font-black uppercase tracking-[.18em] text-violet-100 backdrop-blur">
                <span className="h-2.5 w-2.5 rounded-full bg-academy-gold shadow-[0_0_16px_rgba(239,184,50,.9)]" />
                Diplôme d’État · Bac+2 · Niveau 5 · RNCP 38368
              </span>
              <h1 className="mt-5 max-w-4xl text-4xl font-black tracking-[-.06em] sm:text-5xl lg:text-6xl xl:text-7xl">
                Transformez chaque contact en <span className="text-[#FFD56A]">opportunité.</span>
              </h1>
              <p className="mt-5 max-w-2xl text-lg font-medium leading-8 text-white/70 sm:text-xl">
                Avec le BTS NDRC, apprenez à prospecter, négocier, vendre et fidéliser — sur le terrain, à distance et sur les réseaux sociaux.
              </p>
      ...
      <Section
        ...
        title={<>Une formation vivante pour vous. Un diplôme rassurant pour vos parents.</>}
        intro={<>Le BTS NDRC s’adresse aux jeunes qui aiment le contact, les défis et le numérique — et qui veulent apprendre un métier en le pratiquant vraiment.</>}
      >
        <div className="grid gap-5 lg:grid-cols-2">
          <article className="rounded-[2rem] border border-[#22354C] bg-[#0D1725] p-6 text-white shadow-card sm:p-8">
            <span className="grid h-14 w-14 place-items-center rounded-2xl bg-violet-400/15 text-2xl text-sky-300">↗</span>
            <Eyebrow light>Pour le futur étudiant</Eyebrow>
            <h3 className="mt-4 text-3xl font-black tracking-[-.04em]">Vous aimez convaincre, créer du lien et bouger.</h3>
            <p className="mt-4 leading-7 text-white/62">Le BTS NDRC transforme votre énergie en compétences commerciales : prospecter, négocier, vendre, communiquer en ligne et développer un réseau.</p>
            <div className="mt-6 grid gap-3">
              {['Des missions concrètes dès l’alternance', 'Du terrain, de la visio et des réseaux sociaux', 'Un diplôme qui ouvre de nombreux secteurs'].map((item) => (
          ...
          <article className="rounded-[2rem] border border-academy-line bg-[#FFFDF8] p-6 shadow-card sm:p-8">
            ...
            <h3 className="mt-4 text-3xl font-black tracking-[-.04em]">Vous cherchez un diplôme reconnu et une équipe disponible.</h3>
            ...
            <div className="mt-6 grid gap-3">
              {['Diplôme d’État de niveau 5 — RNCP 38368', 'Sans frais de scolarité pour l’apprenti*', 'Accompagnement de la candidature jusqu’au contrat'].map((item) => (
            ...
            <CTA href={contactHref('Question d’un parent sur le BTS NDRC')} variant="light" className="mt-6">Poser une question à Aurélie →</CTA>
          </article>
        </div>
      </Section>
      ...
      <Section
        id="competences"
        eyebrow="02 — Le commerce nouvelle génération"
        title={<>Vous apprenez à vendre partout où se trouve le client.</>}
      ...
      >
        <div className="grid gap-5 lg:grid-cols-2">
          <article className="rounded-[2rem] border border-academy-line bg-[#FFFDF8] p-6 shadow-card sm:p-8">
            ...
            <div className="mt-6 grid gap-3 sm:grid-cols-2">
              {['Cours en groupe', 'Jeux de rôle commerciaux', 'Vie de promotion', 'Équipe sur place'].map((item) => <p key={item} className="rounded-2xl bg-academy-bg p-4 text-sm font-black">✓ {item}</p>)}
            </div>
          </article>
          <article className="rounded-[2rem] border border-[#392F67] bg-[#171435] p-6 text-white shadow-card sm:p-8">
            <span className="inline-flex rounded-full bg-violet-400/15 px-3 py-2 text-[.62rem] font-black uppercase tracking-[.15em] text-violet-200 ring-1 ring-violet-300/30">◉ Classe virtuelle</span>
        ...
        <div className="mt-5 flex flex-col items-start justify-between gap-4 rounded-[1.7rem] border border-yellow-300 bg-yellow-50 p-5 sm:flex-row sm:items-center">
          ...
          <span className="rounded-full bg-white px-4 py-2 text-xs font-black text-academy-ink ring-1 ring-yellow-300">BTS NDRC · RNCP 38368</span>
        </div>
      </Section>
      ...
      <Section
        ...
        title={<>Apprenez. Pratiquez. Construisez votre CV.</>}
        intro={<>Deux années pour obtenir un diplôme, gagner en assurance, construire votre réseau et valoriser une vraie expérience sur votre CV.</>}
        tone="paper"
      >
        <div className="grid gap-5 lg:grid-cols-[1.2fr_.8fr]">
          <article className="rounded-[2rem] bg-[#0D1725] p-6 text-white shadow-card sm:p-8">
            ...
            <h3 className="mt-4 text-3xl font-black tracking-[-.04em]">À l’école, vous vous entraînez. En entreprise, vous obtenez des résultats.</h3>
            <div className="mt-6 grid gap-3">
              {[
                ['01', 'À l’école', 'Négociation, digital, réseaux, stratégie et mises en situation'],
                ['02', 'En entreprise', 'Prospection, rendez-vous, ventes, fidélisation et outils CRM'],
          ...
          <article className="rounded-[2rem] border border-yellow-300 bg-[#F7E5AC] p-6 shadow-card sm:p-8">
            ...
            <CTA href={contactHref('Question sur le financement du BTS NDRC')} variant="dark" className="mt-6">Poser ma question à Aurélie →</CTA>
        ...
        <article className="mt-5 rounded-[2rem] border border-academy-line bg-academy-bg p-6 shadow-soft">
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {[
              ['Diplôme officiel', 'Diplôme national de niveau 5 — RNCP 38368.'],
      ...
      <Section
        ...
        title={<>Du terrain, du digital et une vraie culture commerciale.</>}
        intro={<>Le programme officiel combine enseignements généraux, négociation-vente, relation client à distance, e-commerce et animation de réseaux.</>}
      >
      ...
      <BtsCompleteInformation course="ndrc" />
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[1.15fr_.85fr]">
            <article className="rounded-[2rem] bg-[#FFFDF8] p-6 text-academy-ink shadow-card">
              ...
              <div className="mt-5 flex flex-wrap gap-2">
                {['Commercial terrain', 'Chargé de clientèle', 'Négociateur', 'Technico-commercial', 'Commercial e-commerce', 'Animateur de réseau', 'Poursuite d’études'].map((job) => <span key={job} className="rounded-full border border-academy-line bg-academy-bg px-4 py-2 text-xs font-black">{job}</span>)}
              </div>
            </article>
            <article className="rounded-[2rem] bg-gradient-to-br from-[#EFB832] to-[#FFD56A] p-6 text-academy-gold-text shadow-gold">
              ...
              <h3 className="mt-3 text-3xl font-black">Prêt à révéler votre fibre commerciale ?</h3>
      ...
      <div id="faq-ndrc">
        <PremiumFAQSection
          ...
          description="Admission, alternance, différence avec le MCO, entreprise, visioconférence, coût ou diplôme : Aurélie répond aux questions du jeune comme à celles de sa famille."
          items={faq}
          contactHref={contactHref('Question sur le BTS NDRC')}
