...
import { a3pConfig, a3pContact, a3pFaq, a3pPrerequisites, a3pProgram } from '@/data/a3p';
...
function priceLabel(value: unknown, fallback: string) {
  const candidate = value ?? fallback;
  if (typeof candidate === 'number') return new Intl.NumberFormat('fr-FR', { style: 'currency', currency: 'EUR', maximumFractionDigits: 0 }).format(candidate);
  const text = String(candidate).trim();
  if (/^\d+(?:[.,]\d+)?$/.test(text)) return new Intl.NumberFormat('fr-FR', { style: 'currency', currency: 'EUR', maximumFractionDigits: 0 }).format(Number(text.replace(',', '.')));
  return text;
}
...
function HeroSession({ sessions }: { sessions: any[] }) {
  ...
  const isFull = next?.status === 'FULL' || Number(next?.seatsLeft) === 0;
  const registrationHref = next?.registrationUrl || (next?.id ? `/contact?formation=a3p-apr&session=${next.id}` : a3pContact('prochaine-session'));
  ...
  return <aside className="rounded-[2rem] border border-white/65 bg-[#FFFDF8] p-5 text-academy-ink shadow-[0_34px_100px_rgba(0,0,0,.34)] sm:p-6">
    ...
    <div className="mt-5 grid grid-cols-2 gap-2.5">
      {[
        ...
        ['Tarif', priceLabel(next?.priceLabel, a3pConfig.priceLabel)],
...
const heroFacts = [
  ...
  ['Durée', '9 semaines', a3pConfig.durationHours],
...
const programPhases = [
  ['01', 'Cadre, risques et fondamentaux', 'Cadre juridique, déontologie, gestion des conflits, risque terroriste, premiers secours, secourisme tactique et prévention incendie.'],
  ['02', 'Préparer et reconnaître', 'Principes de protection, analyse de mission, recueil d’informations, reconnaissance des sites et solutions de repli.'],
  ['03', 'Protéger et se déplacer', 'Techniques de protection, déplacements pédestres, dispositifs véhiculés et communication d’équipe.'],
  ['04', 'Simuler, décider, certifier', 'Missions complètes, scénarios progressifs, briefings, débriefings, évaluations et préparation devant jury.'],
];
...
const examSteps = [
  ['Évaluations théoriques', 'Cadre légal, risques et compétences professionnelles.'],
  ['Préparation de mission', 'Contexte, menaces, itinéraires, sites et moyens nécessaires.'],
  ['Mise en situation', 'Décisions, positionnements, communication et protection.'],
  ['Soutenance devant jury', 'Présentation du dossier et justification des choix.'],
];
...
export function A3pReferencePage({ sessions }: { sessions: any[] }) {
  ...
  return <main className="relative overflow-hidden pb-24 lg:pb-0">
    <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
      ...
      '@graph': [
        { '@type': 'Course', name: 'Formation A3P : devenez agent de protection rapprochée', description: 'Formation officielle TFP A3P à Puget-sur-Argens, 327 heures, RNCP niveau 4.', provider: { '@type': 'Organization', name: 'Intégrale Academy', telephone: a3pConfig.advisor.phone } },
    ...
    <section id="admission" className="bg-[#FFFDF8] px-4 py-14 sm:py-18 lg:py-20"><div className="page-container overflow-hidden rounded-[2.25rem] bg-[#0D1725] p-6 text-white shadow-card sm:p-8 lg:p-10"><div className="grid gap-8 lg:grid-cols-[.72fr_1.28fr]">
      <div><Label light>02 — Admission</Label><h2 className="mt-3 text-3xl font-black tracking-[-.045em] sm:text-4xl">Êtes-vous éligible à la formation A3P&nbsp;?</h2><p className="mt-5 text-sm font-medium leading-7 text-white/62">L’activité est réglementée. Nous contrôlons chaque point pour éviter qu’un dossier incomplet bloque votre rentrée.</p><CTA href={a3pContact('verification-prerequis')} variant="gold" className="mt-6">Vérifier mes prérequis →</CTA></div>
    ...
    <Section label="03 — Compétences" title={<>Ce que vous saurez faire <span className="decoration-academy-gold decoration-[.16em] underline underline-offset-[-.03em]">sur le terrain.</span></>} intro="Dix compétences métier regroupées en cinq piliers lisibles. Le détail reste accessible sans transformer la page en catalogue de cartes.">
    ...
    <Section id="programme" label="04 — Programme" title={<>327 heures pour devenir <span className="decoration-academy-gold decoration-[.16em] underline underline-offset-[-.03em]">opérationnel.</span></>} intro="Le référentiel complet est conservé. Pour faciliter la lecture, les quinze modules sont organisés en quatre grandes phases pédagogiques." tone="stone">
      <div className="grid items-start gap-5 lg:grid-cols-[.62fr_1.38fr]">
        <aside className="rounded-[2rem] bg-academy-gold p-7 shadow-soft lg:sticky lg:top-5"><span className="rounded-full bg-[#11100D] px-3 py-1.5 text-[.62rem] font-black uppercase tracking-[.14em] text-academy-gold">Formation réglementée</span><p className="mt-8 text-6xl font-black tracking-[-.06em]">327 h</p><p className="mt-3 font-black text-[#4F3B07]">Environ 9 semaines<br />100 % en présentiel</p><ul className="mt-8 space-y-2 text-sm font-bold text-[#4F3B07]"><li>✓ Apports théoriques</li><li>✓ Exercices pratiques</li><li>✓ Travaux en groupe</li><li>✓ Mises en situation</li><li>✓ Préparation au jury</li></ul><CTA href={a3pConfig.programPdfUrl} className="mt-8 w-full">Télécharger le programme →</CTA></aside>
        <div className="space-y-3">{programPhases.map(([number, title, body]) => <article key={title} className="rounded-[1.65rem] border border-academy-line bg-[#FFFDF8] p-5 shadow-soft sm:p-6"><div className="flex gap-4"><span className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl border border-academy-line bg-academy-bg text-xs font-black">{number}</span><div><h3 className="text-xl font-black tracking-[-.03em]">{title}</h3><p className="mt-2 text-sm font-semibold leading-7 text-academy-muted">{body}</p></div></div></article>)}
          <details className="group overflow-hidden rounded-[1.65rem] border border-academy-line bg-[#FFFDF8] shadow-soft"><summary className="flex cursor-pointer list-none items-center justify-between gap-4 p-5 font-black sm:p-6"><span>Voir le détail des 15 modules réglementaires</span><span className="grid h-9 w-9 place-items-center rounded-full bg-academy-ink text-white transition group-open:rotate-45">+</span></summary><div className="space-y-3 border-t border-academy-line bg-academy-bg p-4 sm:p-5">{a3pProgram.map((item) => <details key={item.title} className="group/module rounded-2xl border border-academy-line bg-white p-4"><summary className="flex cursor-pointer list-none justify-between gap-3 text-sm font-black"><span>{item.title}</span><span className="text-yellow-700 transition group-open/module:rotate-45">+</span></summary><div className="mt-3 text-xs font-semibold leading-6 text-academy-muted"><p>{item.description}</p><ul className="mt-3 grid gap-2 sm:grid-cols-2">{item.items?.map((point) => <li key={point} className="rounded-xl bg-academy-bg px-3 py-2">✓ {point}</li>)}</ul></div></details>)}</div></details>
        </div>
      </div>
    </Section>
    ...
    <Section id="dates-tarif" label="07 — Dates & financement" title="Planifiez votre rentrée." intro="Les dates, examens et places restantes proviennent du planning administrable de l’école.">
      <div className="grid gap-5 lg:grid-cols-[1.05fr_.95fr]">
        <div className="rounded-[2rem] border border-academy-line bg-[#FFFDF8] p-6 shadow-card sm:p-8"><h3 className="text-2xl font-black tracking-[-.04em]">Prochaines formations A3P</h3><div className="mt-6 space-y-3">{sessions.length ? sessions.map((session) => <article key={session.id} className="rounded-[1.5rem] border border-emerald-200 bg-emerald-50/70 p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-lg font-black">{formatDate(session.startDate)} <span className="text-yellow-700">→</span> {formatDate(session.endDate)}</p><p className="mt-1 text-xs font-bold text-academy-muted">{session.examDate ? `Examen le ${formatDate(session.examDate)}` : 'Date d’examen à confirmer'} · {session.location || a3pConfig.location}</p></div><span className="rounded-full border border-orange-300 bg-orange-50 px-3 py-1.5 text-[.65rem] font-black text-orange-800">{seatsLabel(session.seatsLeft)}</span></div><div className="mt-4"><CTA href={session.registrationUrl || `/contact?formation=a3p-apr&session=${session.id}`}>Réserver cette session →</CTA></div></article>) : <div className="rounded-[1.5rem] border border-dashed border-academy-line bg-academy-bg p-6 text-sm font-bold text-academy-muted">La prochaine date est en cours de confirmation. Demandez à être rappelé pour recevoir le planning.</div>}</div><div className="mt-5 flex flex-wrap gap-3"><CTA href="/planning?formation=a3p" variant="light">Voir tout le planning</CTA><CTA href={a3pContact('alerte-session')} variant="light">Recevoir les prochaines dates</CTA></div></div>
        <aside className="relative overflow-hidden rounded-[2rem] bg-[#0D1725] p-6 text-white shadow-card sm:p-8"><div className="absolute -right-24 -top-24 h-60 w-60 rounded-full bg-academy-gold/15 blur-2xl" /><Label light>Tarif de la formation</Label><p className="relative mt-5 text-5xl font-black tracking-[-.05em] sm:text-6xl">{priceLabel(next?.priceLabel, a3pConfig.priceLabel)}</p><p className="relative mt-4 text-sm font-medium leading-7 text-white/62">Formation complète de {a3pConfig.durationHours}. Plusieurs solutions peuvent être étudiées selon votre situation et la décision du financeur.</p><div className="relative mt-6 grid grid-cols-2 gap-2">{['CPF selon éligibilité', 'France Travail', 'Employeur / OPCO', 'Paiement échelonné'].map((item) => <span key={item} className="rounded-2xl border border-white/10 bg-[#182537] px-3 py-3 text-[.68rem] font-black">{item}</span>)}</div><CTA href={a3pContact('financement-a3p')} variant="gold" className="relative mt-6">Étudier mon financement →</CTA></aside>
      </div>
      <div className="mt-5 flex flex-col items-start justify-between gap-5 rounded-[1.75rem] bg-[linear-gradient(135deg,#F8E6B2,#F1BB38)] p-6 shadow-soft sm:flex-row sm:items-center"><div><Label>Financement</Label><h3 className="mt-2 text-2xl font-black tracking-[-.03em]">Simulez votre reste à charge.</h3><p className="mt-1 text-xs font-semibold text-[#4F3B07]">Obtenez une estimation rapide avant d’échanger avec notre équipe.</p></div><CTA href="/financements#simulateur">Lancer le simulateur →</CTA></div>
    </Section>
    ...
    <section className="bg-[#FFFDF8] px-4 py-14 sm:py-18 lg:py-20"><div className="page-container">
      ...
      <div className="mt-14 grid gap-5 lg:grid-cols-[1.05fr_.95fr]"><div className="rounded-[2rem] bg-[#0D1725] p-7 text-white shadow-card"><Label light>08 — Débouchés</Label><h2 className="mt-3 text-3xl font-black tracking-[-.045em] sm:text-4xl">Après le TFP A3P.</h2><div className="mt-6 flex flex-wrap gap-2">{jobs.map((job) => <span key={job} className="rounded-full border border-white/12 bg-[#182537] px-4 py-2 text-[.68rem] font-black">{job}</span>)}</div><p className="mt-6 text-xs font-medium leading-6 text-white/55">La certification ne garantit pas un emploi. Le recrutement dépend aussi de l’expérience, du réseau, de la mobilité et des compétences complémentaires.</p></div><div className="rounded-[2rem] border border-academy-line bg-white p-7 shadow-soft"><Label>Rémunération</Label><h2 className="mt-3 text-2xl font-black tracking-[-.04em]">Un niveau variable selon les missions.</h2><p className="mt-4 text-sm font-semibold leading-7 text-academy-muted">Le type de mission, la durée, le niveau de risque et l’expérience influencent fortement la rémunération.</p><div className="mt-5 flex flex-wrap gap-2">{salaryFactors.map((factor) => <span key={factor} className="rounded-full bg-[#F3EEE5] px-3 py-2 text-[.65rem] font-black">{factor}</span>)}</div></div></div>
    ...
    <Section label="09 — Intégrale Academy" title="Un cadre sérieux pour un métier exigeant." intro="Des éléments concrets et vérifiables, directement liés au parcours A3P." tone="stone">
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{[
        ['Agrément', 'Centre agréé ADEF A3P', `N° ${a3pConfig.adefApproval}`],
        ['Intensité', '327 heures de formation', 'Parcours réglementé et progressif'],
    ...
    <div id="faq-a3p"><PremiumFAQSection badge="10 — FAQ A3P" title="Tout savoir avant de vous inscrire" description="Retrouvez les réponses aux questions les plus fréquentes sur la formation A3P, les prérequis CNAPS, le financement, l’hébergement et l’examen." items={a3pFaq} contactHref={a3pContact('question-faq-a3p')} /></div>
    ...
    <div className="fixed inset-x-0 bottom-0 z-40 border-t border-white/70 bg-white/92 p-3 shadow-[0_-18px_40px_rgba(17,17,17,.12)] backdrop-blur lg:hidden"><div className="mx-auto flex max-w-md gap-2"><CTA href={a3pConfig.advisor.phoneHref} className="flex-1">Appeler</CTA><CTA href={a3pContact('mobile-dossier-a3p')} variant="gold" className="flex-1">Recevoir le dossier</CTA></div></div>
  </main>;
}
