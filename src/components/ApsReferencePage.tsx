...
const jobs = ['Agent de prévention et de sécurité', 'Agent de surveillance', 'Agent rondier', 'Agent mobile / intervenant', 'Agent de sécurité événementielle', 'Agent arrière-caisse', 'Agent de contrôle d’accès', 'Opérateur de vidéoprotection selon poste et habilitations'];
...
export function ApsReferencePage({ sessions }: { sessions: any[] }) {
  ...
  return <main className="relative overflow-hidden pb-24 lg:pb-0">
    ...
    <nav aria-label="Sommaire de la formation" className="sticky top-0 z-30 hidden border-b border-academy-line bg-[#FFFDF8]/95 px-4 py-3 backdrop-blur lg:block"><div className="page-container flex items-center justify-between gap-5"><span className="text-xs font-black">TFP APS</span><div className="flex items-center gap-5 text-xs font-extrabold text-academy-muted">{[['Métier','#metier'],['Admission','#admission'],['Format hybride','#hybride'],['Pratique','#pratique'],['Programme','#programme'],['Examen','#examen'],['Dates & tarifs','#dates-tarifs'],['Débouchés','#debouches'],['FAQ','#faq-aps']].map(([label,href]) => <Link key={href} href={href} className="transition hover:text-academy-ink">{label}</Link>)}</div><CTA href={sessionHref(next)} variant="gold" className="min-h-10 px-4 py-2">Je m’inscris</CTA></div></nav>
