...
export function ApsReferencePage({ sessions }: { sessions: any[] }) {
  ...
  return <main className={`${styles.page} relative overflow-hidden pb-24 lg:pb-0`}>
    ...
    <section className={`${styles.hero} relative min-h-[720px] px-4 text-white`}><Image src="/images/aps/aps-hero-round.jpg" alt="Exercice pratique de ronde de sécurité pendant la formation APS" fill priority sizes="100vw" className={styles.heroPhoto}/><div className={styles.heroOverlay}/><div className="page-container relative flex min-h-[720px] items-center py-16 sm:py-20 lg:py-24"><div className="max-w-3xl"><span className={`${styles.heroBadge} inline-flex items-center gap-2 rounded-full border border-academy-gold/45 bg-black/25 px-4 py-2 text-[.68rem] font-black uppercase tracking-[.2em] text-[#F7D878] backdrop-blur-md`}><span className="h-2.5 w-2.5 rounded-full bg-academy-gold shadow-[0_0_16px_rgba(238,184,47,.9)]"/>TFP APS · RNCP 36648 · niveau 3</span><p className="mt-7 text-sm font-black uppercase tracking-[.22em] text-sky-200">Formation agent de prévention et de sécurité</p><h1 className={`${styles.heroTitle} mt-3 max-w-4xl text-5xl font-black tracking-[-.06em] sm:text-6xl lg:text-7xl xl:text-[5.5rem] xl:leading-[.94]`}>Apprenez le métier <span className={styles.heroTitleAccent}>sur le terrain.</span></h1><p className="mt-6 max-w-2xl text-lg font-medium leading-8 text-white/78 sm:text-xl">Rondes, contrôle d’accès, palpation, inspection des bagages et gestion d’incidents&nbsp;: entraînez-vous aux situations que vous rencontrerez réellement en poste.</p><div className="mt-8 flex flex-col gap-3 sm:flex-row"><CTA href={apsContact('éligibilité')} variant="gold">Vérifier mon éligibilité →</CTA><CTA href="tel:0422470768" variant="outline">Parler à un conseiller</CTA></div><div className="mt-8 flex flex-wrap gap-2 text-xs font-bold text-white/80"><span className="rounded-full border border-white/20 bg-black/25 px-3 py-2 backdrop-blur">✓ 175 heures</span><span className="rounded-full border border-white/20 bg-black/25 px-3 py-2 backdrop-blur">✓ SST inclus</span><span className="rounded-full border border-white/20 bg-black/25 px-3 py-2 backdrop-blur">✓ CNAPS accompagné</span><span className="rounded-full border border-white/20 bg-black/25 px-3 py-2 backdrop-blur">✓ Financements possibles</span></div></div></div><div className={`${styles.facts} page-container relative -mt-28 grid overflow-hidden rounded-[1.6rem] border border-white/15 bg-[#0A1421]/85 text-white backdrop-blur-xl sm:grid-cols-2 lg:grid-cols-6`}>{heroFacts.map(([key,value,detail]) => <div key={key} className={`${styles.fact} border-b border-white/10 p-4 last:border-b-0 sm:border-r lg:border-b-0`}><p className="text-[.58rem] font-black uppercase tracking-[.18em] text-white/42">{key}</p><p className="mt-1 font-black text-white">{value}</p><p className="mt-1 text-[.68rem] font-semibold leading-4 text-white/48">{detail}</p></div>)}</div></section>
    ...
    >
      <div className="grid overflow-hidden rounded-[2rem] border border-academy-line shadow-card lg:grid-cols-2">
        ...
        <article className="bg-white p-7 lg:p-9">
          <span className="text-5xl font-black text-emerald-700">124 h min.</span>
          <h3 className="mt-2 text-2xl font-black">en présentiel</h3>
          <div className="mt-6 grid gap-3">{['63,5 h de pratique','60,5 h de théorie en présentiel','Rondes et mises en situation','Préparation à l’examen'].map(item => <p key={item} className="flex gap-3 font-bold text-academy-muted"><span className="text-emerald-600">✓</span>{item}</p>)}</div>
        </article>
      </div>
    ...
    <Section id="programme" eyebrow="05 — Programme" title={<>Les 14 UV du programme officiel, sans raccourci.</>} intro={<>Programme CPNEFP version V3.2 mis à jour le 23 juillet 2026 : 41 heures de socle de base et 134 heures de spécialité APS, soit 175 heures au total.</>}>
      ...
      <div className="grid gap-3 lg:grid-cols-2">{program.map(([number,title,duration,delivery,text],index) => <details key={number} open={index===0} className={`${styles.programItem} group rounded-[1.4rem] border p-4 ${number === 'UV 6' ? 'border-emerald-300 bg-emerald-50' : number === 'UV 1' || number === 'UV 7' ? 'border-academy-gold/70 bg-[#FFFDF8]' : 'border-academy-line bg-[#FFFDF8]'}`}><summary className="flex cursor-pointer list-none items-center gap-3"><span className={`grid h-9 min-w-14 shrink-0 place-items-center rounded-full px-2 text-xs font-black ${number === 'UV 6' ? 'bg-emerald-200 text-emerald-900' : 'bg-academy-bg text-academy-ink'}`}>{number}</span><strong className="min-w-0 flex-1">{title}</strong><span className="hidden rounded-full bg-academy-bg px-3 py-1 text-xs font-black text-academy-muted sm:inline-flex">{duration}</span><span className="grid h-8 w-8 shrink-0 place-items-center rounded-full border border-academy-line font-black transition group-open:rotate-45">+</span></summary><div className="mt-4 border-t border-academy-line/70 pt-4"><div className="flex flex-wrap gap-2"><span className="rounded-full bg-academy-ink px-3 py-1 text-xs font-black text-white sm:hidden">{duration}</span><span className="rounded-full bg-sky-50 px-3 py-1 text-xs font-black text-sky-800 ring-1 ring-sky-200">{delivery}</span></div><p className="mt-3 text-sm leading-6 text-academy-muted">{text}</p></div></details>)}</div>
    ...
    <Section id="reperes-rncp" eyebrow="10 — Repères officiels" title={<>Certification, conditions d’exercice et données d’insertion.</>} intro={<>Ces informations complètent la présentation commerciale avec les repères figurant dans la fiche RNCP36648 fournie.</>}>
      <div className="grid gap-5 lg:grid-cols-[1.15fr_.85fr]">
        <article className="rounded-[2rem] border border-academy-line bg-white p-6 shadow-soft lg:p-8">
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-2">{workConditions.map(item => <div key={item} className="flex gap-3 rounded-2xl bg-academy-bg p-4 text-sm font-bold leading-6 text-academy-muted"><span className="mt-0.5 text-emerald-700">✓</span><span>{item}</span></div>)}</div>
        </article>
