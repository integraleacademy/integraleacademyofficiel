...
function CTA({
  ...
  return (
    <Link
      href={href}
      className={`inline-flex min-h-12 items-center justify-center rounded-full px-5 py-3 text-center text-sm font-black transition hover:-translate-y-0.5 focus:outline-none focus:ring-4 focus:ring-academy-gold/25 ${variants[variant]} ${className}`}
    >
      {children}
    </Link>
  );
...
function Eyebrow({ children, light = false }: { children: ReactNode; light?: boolean }) {
  return (
    <p className={`text-[.66rem] font-black uppercase tracking-[.24em] ${light ? 'text-orange-300' : 'text-orange-700'}`}>
      {children}
    </p>
  );
}
  ...
  return (
    <main className="relative overflow-hidden pb-24 lg:pb-0">
      ...
      <section className="relative isolate overflow-hidden bg-[#0D1725] px-4 pb-9 pt-10 text-white sm:pt-14 lg:pt-16">
        <div className="absolute inset-0 -z-10 bg-[radial-gradient(circle_at_12%_12%,rgba(238,184,47,.25),transparent_31%),radial-gradient(circle_at_88%_20%,rgba(239,68,68,.20),transparent_29%),linear-gradient(135deg,#080D15_0%,#121B2A_55%,#2A160F_100%)]" />
        ...
        <div className="page-container">
          <nav className="mb-7 flex flex-wrap items-center gap-2 text-xs font-bold text-white/55" aria-label="Fil d’Ariane">
            ...
            <span className="text-orange-200">{config.label}</span>
          </nav>

          <div className="grid items-center gap-8 lg:grid-cols-[1.08fr_.92fr] lg:gap-12">
            <div>
              <span className="inline-flex items-center gap-2 rounded-full border border-orange-300/35 bg-orange-400/10 px-4 py-2 text-[.68rem] font-black uppercase tracking-[.2em] text-orange-200">
                <span className="h-2.5 w-2.5 rounded-full bg-orange-400 shadow-[0_0_16px_rgba(251,146,60,.9)]" />
              ...
              <div className="mt-7 flex flex-col gap-3 sm:flex-row">
                <CTA href={contactHref(config, 'prochaines dates')} variant="gold">Recevoir les prochaines dates →</CTA>
            ...
            <aside className="rounded-[2rem] border border-white/65 bg-[#FFFDF8] p-5 text-academy-ink shadow-[0_34px_100px_rgba(0,0,0,.34)] sm:p-7">
              <span className="rounded-full bg-[#0D1725] px-3 py-1.5 text-[.62rem] font-black uppercase tracking-[.16em] text-orange-300">Votre parcours</span>
              <h2 className="mt-5 text-3xl font-black tracking-[-.04em] sm:text-4xl">{config.label}</h2>
              <p className="mt-2 text-lg font-extrabold text-orange-700">{config.role}</p>
      ...
      <nav aria-label="Sommaire de la formation" className="sticky top-0 z-30 hidden border-b border-academy-line bg-[#FFFDF8]/95 px-4 py-3 backdrop-blur lg:block">
        <div className="page-container flex items-center justify-between gap-5">
          ...
          <CTA href={contactHref(config, 'inscription')} variant="gold" className="min-h-10 px-4 py-2">Je m’inscris</CTA>
        </div>
      </nav>
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          {config.missions.map((mission, index) => (
            <article key={mission.title} className={`rounded-[1.8rem] border p-6 shadow-soft ${index === 1 ? 'border-[#26384F] bg-[#0D1725] text-white' : 'border-academy-line bg-[#FFFDF8]'}`}>
              <span className={`text-5xl font-black ${index === 1 ? 'text-orange-300' : 'text-orange-600'}`}>0{index + 1}</span>
          ...
          {config.prerequisites.map((item, index) => (
            <article key={item.title} className="rounded-[1.7rem] border border-academy-line bg-white p-5 shadow-soft">
              <div className="flex items-start gap-4">
                <span className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-orange-100 font-black text-orange-700">{index + 1}</span>
                <div><h3 className="text-xl font-black">{item.title}</h3><p className="mt-2 leading-7 text-academy-muted">{item.text}</p></div>
              </div>
            </article>
          ))}
        </div>
        <div className="mt-8 flex flex-col items-start justify-between gap-5 rounded-[1.7rem] border border-orange-200 bg-orange-50 p-5 sm:flex-row sm:items-center">
          <div><p className="font-black text-orange-950">Vous ne savez pas si vos justificatifs sont suffisants ?</p><p className="mt-1 text-sm font-semibold leading-6 text-orange-900/70">Envoyez votre dossier : l’équipe vérifie votre situation avant tout engagement.</p></div>
      ...
      <Section id="programme" eyebrow="Programme réglementaire" title={`${config.duration} pour maîtriser votre fonction.`} intro={config.durationDetail}>
          ...
          {config.program.map((item, index) => (
            <details key={item.title} open={index === 0} className="group rounded-[1.6rem] border border-academy-line bg-[#FFFDF8] p-5 shadow-soft">
              <summary className="flex cursor-pointer list-none items-center gap-4">
                <span className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-[#0D1725] font-black text-orange-300">{String(index + 1).padStart(2, '0')}</span>
                ...
                <span className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-academy-gold font-black transition group-open:rotate-45">+</span>
              </summary>
        ...
        <div className="mt-8 rounded-[1.7rem] border border-academy-line bg-[#EFE7D9] p-5">
          ...
          <a href={ssiapOfficialReference} target="_blank" rel="noopener noreferrer" className="mt-3 inline-flex font-black text-orange-700 underline decoration-orange-300 underline-offset-4">Consulter le texte sur Légifrance ↗</a>
        </div>
      </Section>
      ...
      <Section id="validation" eyebrow="Validation" title="Des attendus clairs, travaillés pendant le parcours." intro="La formation alterne apports réglementaires, exercices, cas concrets et préparation aux modalités de validation." tone="dark">
        <div className="grid gap-4 lg:grid-cols-3">
          {config.assessment.map((step, index) => (
            <article key={step.title} className={`rounded-[1.8rem] border p-6 ${index === 1 ? 'border-orange-300/35 bg-orange-400/10' : 'border-white/10 bg-white/6'}`}>
              <span className="text-5xl font-black text-orange-300">0{index + 1}</span>
      ...
      <section id="dates-tarifs" className="scroll-mt-24 bg-[#EFE7D9] px-4 py-14 sm:py-16">
        <div className="page-container overflow-hidden rounded-[2.2rem] bg-[#0D1725] p-6 text-white shadow-card sm:p-9 lg:grid lg:grid-cols-[1fr_auto] lg:items-center lg:gap-10">
          ...
          <div className="mt-7 flex flex-col gap-3 lg:mt-0">
            <CTA href={contactHref(config, 'dates et tarif')} variant="gold">Recevoir les dates →</CTA>
      ...
      <div className="fixed inset-x-0 bottom-0 z-40 border-t border-academy-line bg-[#FFFDF8]/96 p-3 pb-[calc(.75rem+env(safe-area-inset-bottom))] shadow-[0_-12px_40px_rgba(0,0,0,.12)] backdrop-blur lg:hidden">
        <div className="mx-auto flex max-w-lg gap-2"><CTA href="tel:0422470768" variant="light" className="min-w-0 flex-1 px-3">Appeler</CTA><CTA href={contactHref(config, 'inscription')} variant="gold" className="min-w-0 flex-[1.4] px-3">Recevoir les dates</CTA></div>
      </div>
    </main>
  );
}
