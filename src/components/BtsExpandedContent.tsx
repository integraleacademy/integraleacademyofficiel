    ...
    <ul className="grid gap-2.5">
      {items.map((item) => (
        <li key={item} className={`flex gap-3 text-sm leading-6 ${dark ? 'text-white/72' : 'text-academy-muted'}`}>
          <span className={`mt-1 grid h-5 w-5 shrink-0 place-items-center rounded-full text-[.62rem] font-black ${dark ? 'bg-bts-400 text-bts-950' : 'bg-bts-100 text-bts-800'}`}>
            ✓
          </span>
          <span>{item}</span>
        </li>
    ...
    <section id="avantages-bts" className="scroll-mt-24 bg-[#081522] px-4 py-14 text-white sm:py-16 lg:py-20">
      <div className="page-container">
        <div className="mx-auto max-w-4xl text-center">
          <span className="inline-flex rounded-full border border-bts-200/40 bg-bts-200/10 px-4 py-2 text-[.65rem] font-black uppercase tracking-[.2em] text-bts-200">
            Deux avantages exceptionnels inclus
          </span>
          <h2 className="mt-5 text-3xl font-black tracking-[-.05em] sm:text-4xl lg:text-5xl">
            Votre {courseName}, avec <span className="text-bts-200">l’iPad et Londres.</span>
          </h2>
        ...
        <div className="mt-10 grid gap-5 lg:grid-cols-2">
          <article className="relative overflow-hidden rounded-[2rem] border border-bts-300/25 bg-gradient-to-br from-bts-900 to-bts-950 p-7 shadow-[0_28px_80px_rgba(0,0,0,.28)] sm:p-9">
            <div className="absolute -right-16 -top-16 h-48 w-48 rounded-full bg-bts-400/15 blur-2xl" />
            <div className="relative flex items-start justify-between gap-5">
              <span className="grid h-20 w-20 shrink-0 place-items-center rounded-[1.5rem] bg-bts-300 text-bts-950 shadow-soft">
                <TabletIcon />
              </span>
              <span className="rounded-full bg-white/10 px-3 py-2 text-[.62rem] font-black uppercase tracking-[.15em] text-bts-100 ring-1 ring-white/15">
                Dès la signature
              </span>
            </div>
            <p className="relative mt-7 text-[.68rem] font-black uppercase tracking-[.18em] text-bts-300">Votre outil de travail</p>
          ...
          <article className="relative overflow-hidden rounded-[2rem] border border-bts-200/35 bg-gradient-to-br from-bts-200 via-bts-300 to-bts-400 p-7 text-bts-950 shadow-card sm:p-9">
            <div className="absolute -bottom-20 -right-12 h-56 w-56 rounded-full bg-white/25 blur-2xl" />
            <div className="relative flex items-start justify-between gap-5">
              <span className="grid h-20 w-20 shrink-0 place-items-center rounded-[1.5rem] bg-bts-950 text-bts-200 shadow-[0_16px_40px_rgba(36,23,7,.22)]">
                <LondonIcon />
              </span>
              <span className="rounded-full bg-bts-950 px-3 py-2 text-[.62rem] font-black uppercase tracking-[.15em] text-white shadow-soft">
                100 % pris en charge
              </span>
            </div>
            <p className="relative mt-7 text-[.68rem] font-black uppercase tracking-[.18em] text-bts-800">Une expérience de promotion</p>
            <h3 className="relative mt-3 text-3xl font-black tracking-[-.04em] sm:text-4xl">Un voyage à Londres en 2ᵉ année.</h3>
            <p className="relative mt-4 max-w-xl text-base font-semibold leading-7 text-bts-950/75">
    ...
    <section id="guide-complet" className="scroll-mt-24 bg-[#FFFDF8] px-4 py-14 text-academy-ink sm:py-16 lg:py-20">
      <div className="page-container">
        <div className="grid gap-6 lg:grid-cols-[1fr_auto] lg:items-end">
          <div>
            <p className="text-[.66rem] font-black uppercase tracking-[.24em] text-bts-700">Le guide complet de la formation</p>
        ...
        <div className="mt-9 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {[
            ...
            <article key={label} className="rounded-[1.5rem] border border-academy-line bg-academy-bg p-5 shadow-soft">
              <p className="text-[.62rem] font-black uppercase tracking-[.16em] text-bts-700">{label}</p>
        ...
        <div className="mt-10 grid gap-5 lg:grid-cols-2">
          <article className="rounded-[2rem] bg-[#0D1725] p-6 text-white shadow-card sm:p-8">
            <p className="text-[.66rem] font-black uppercase tracking-[.2em] text-bts-300">Objectifs de la formation</p>
          ...
          <article className="rounded-[2rem] border border-academy-line bg-white p-6 shadow-card sm:p-8">
            <p className="text-[.66rem] font-black uppercase tracking-[.2em] text-bts-700">Profil et qualités attendues</p>
        ...
        <div className="mt-12">
          <p className="text-[.66rem] font-black uppercase tracking-[.22em] text-bts-700">Missions en entreprise</p>
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {data.missions.map((mission, index) => (
              <article key={mission} className="grid grid-cols-[2.5rem_1fr] gap-3 rounded-[1.35rem] border border-academy-line bg-academy-bg p-4">
                <span className="grid h-10 w-10 place-items-center rounded-xl bg-white text-[.65rem] font-black text-bts-700 shadow-soft">
        ...
        <div className="mt-12 rounded-[2rem] border border-academy-line bg-academy-bg p-5 sm:p-7 lg:p-9">
          <p className="text-[.66rem] font-black uppercase tracking-[.22em] text-bts-700">Programme professionnel détaillé</p>
            ...
            {data.competencyBlocks.map((block, index) => (
              <details key={block.title} open={index === 0} className="group rounded-[1.5rem] border border-academy-line bg-white p-5 shadow-soft">
                <summary className="flex cursor-pointer list-none items-center gap-3">
                  <span className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-[#0D1725] text-xs font-black text-bts-200">
                    {String(index + 1).padStart(2, '0')}
                  </span>
                  <strong className="min-w-0 flex-1 leading-6">{block.title}</strong>
                  <span className="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-bts-200 font-black transition group-open:rotate-45">+</span>
                </summary>
        ...
        <div id="examens" className="mt-12 scroll-mt-24">
          <div className="grid gap-5 lg:grid-cols-[.68fr_1.32fr]">
            <article className="rounded-[2rem] bg-[#0D1725] p-6 text-white shadow-card sm:p-8">
              <p className="text-[.66rem] font-black uppercase tracking-[.22em] text-bts-300">L’examen du BTS</p>
                    ...
                    {data.examUnits.map((unit) => (
                      <tr key={unit.title} className="border-t border-academy-line">
                        ...
                        <td className="px-5 py-4 text-center">
                          <span className="inline-grid h-9 min-w-9 place-items-center rounded-full bg-bts-200 px-2 text-xs font-black text-bts-950">{unit.coefficient}</span>
                        </td>
        ...
        <div className="mt-12 grid gap-5 lg:grid-cols-2">
          <article className="rounded-[2rem] border border-academy-line bg-white p-6 shadow-card sm:p-8">
            <p className="text-[.66rem] font-black uppercase tracking-[.22em] text-bts-700">Admission et alternance</p>
            ...
            <div className="mt-6 rounded-2xl border border-bts-200 bg-bts-50 p-5 text-bts-950">
              <p className="font-black">Pas encore d’entreprise à la rentrée 2026 ?</p>
              <p className="mt-2 text-sm font-medium leading-6 text-bts-900/75">
          ...
          <article className="rounded-[2rem] border border-academy-line bg-academy-bg p-6 shadow-card sm:p-8">
            <p className="text-[.66rem] font-black uppercase tracking-[.22em] text-bts-700">Entreprises d’accueil</p>
        ...
        {data.specificPanel && (
          <article className="mt-5 rounded-[2rem] border border-bts-300 bg-bts-50 p-6 shadow-soft sm:p-8">
            <div className="grid gap-6 lg:grid-cols-[.8fr_1.2fr] lg:items-center">
              <div>
                <p className="text-[.66rem] font-black uppercase tracking-[.22em] text-bts-800">À savoir</p>
                <h3 className="mt-3 text-3xl font-black tracking-[-.04em] text-bts-950">{data.specificPanel.title}</h3>
                <p className="mt-4 text-sm font-medium leading-7 text-bts-950/70">{data.specificPanel.body}</p>
              </div>
              <div className="rounded-[1.5rem] bg-white/65 p-5 ring-1 ring-bts-300"><CheckList items={data.specificPanel.items} /></div>
            </div>
          </article>
        )}

        {data.campusEquipment && (
          <article className="mt-5 overflow-hidden rounded-[2rem] bg-bts-950 p-6 text-white shadow-card sm:p-8">
            <div className="grid gap-7 lg:grid-cols-[.7fr_1.3fr] lg:items-center">
              <div>
                <p className="text-[.66rem] font-black uppercase tracking-[.22em] text-bts-300">Équipements de l’école</p>
        ...
        <div className="mt-12 grid gap-5 lg:grid-cols-2">
          <article className="rounded-[2rem] bg-[#0D1725] p-6 text-white shadow-card sm:p-8">
            <p className="text-[.66rem] font-black uppercase tracking-[.22em] text-bts-300">Après le diplôme</p>
          ...
          <article className="rounded-[2rem] border border-academy-line bg-white p-6 shadow-card sm:p-8">
            <p className="text-[.66rem] font-black uppercase tracking-[.22em] text-bts-700">Poursuite d’études</p>
