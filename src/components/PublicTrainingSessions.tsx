...
export function PublicTrainingSessions({ sessions, title = 'Prochaines dates', intro = 'Dates, places et informations issues de l’administration.', displayLimit = 3, moreHref = '/planning#sessions' }: { sessions: PublicTrainingSession[]; title?: string; intro?: string; displayLimit?: number; moreHref?: string }) {
  ...
  return <section className="mx-auto max-w-7xl px-4 py-10 sm:py-12">
    ...
    {rows.length ? <><div className="space-y-3">{visibleRows.map((session, index) => { const badge = seatsBadge(computedSeats(session)); const details = [
    ...
    </article>; })}</div>{hiddenCount > 0 ? <div className="mt-6 text-center"><Link href={moreHref} className="inline-flex rounded-full border border-academy-line bg-white px-6 py-3 text-sm font-black text-academy-ink shadow-soft transition hover:-translate-y-0.5 hover:border-academy-gold/60 hover:bg-stone-50">Voir plus de dates ({hiddenCount})</Link></div> : null}</> : <div className="rounded-[2rem] border border-dashed border-academy-line bg-white p-8 text-center shadow-soft"><p className="text-2xl font-black">Aucune date disponible actuellement</p><p className="mx-auto mt-3 max-w-xl text-academy-muted">Les dates seront affichées ici dès qu’une session sera ouverte dans l’administration.</p><Link href="/contact?motif=alerte-planning" className="mt-6 inline-flex rounded-full bg-academy-gold px-5 py-3 text-sm font-black text-academy-gold-text">Être prévenu</Link></div>}
  </section>;
}
