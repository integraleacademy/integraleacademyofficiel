...
export function PublicTrainingSessions({ sessions, title = 'Prochaines dates', intro = 'Dates, places et informations issues de l’administration.', displayLimit = 3, moreHref = '/planning#sessions', emptyTitle = 'Les prochaines dates sont en cours de programmation', emptyText = 'Laissez-nous vos coordonnées pour recevoir les nouvelles dates dès leur ouverture et vérifier dès maintenant votre éligibilité à la formation.', emptyHref = '/contact?motif=alerte-planning', emptyCta = 'Être informé des prochaines dates' }: { sessions: PublicTrainingSession[]; title?: React.ReactNode; intro?: string; displayLimit?: number; moreHref?: string; emptyTitle?: string; emptyText?: string; emptyHref?: string; emptyCta?: string }) {
  ...
  return <section className="page-container py-10 sm:py-12">
    ...
    {rows.length ? <div className="space-y-3">{rows.map((session, index) => { const badge = seatsBadge(computedSeats(session)); const details = [
      ...
      <div className="grid gap-3 xl:grid-cols-[minmax(13rem,.85fr)_minmax(0,1.7fr)_minmax(13rem,.75fr)] xl:items-center">
        ...
        <div className="flex flex-col gap-2 sm:flex-row xl:flex-col">{session.status === 'FULL' ? <p className="rounded-2xl border border-amber-200 bg-amber-50 p-3 text-sm font-black leading-6 text-amber-900">Cette session est complète. Rejoignez la liste d’attente ou recevez les prochaines dates.</p> : <Link href={primaryHref(session)} className="inline-flex flex-1 items-center justify-center rounded-full bg-academy-ink px-5 py-3 text-center text-sm font-black text-white shadow-[0_12px_28px_rgba(23,19,13,.16)] transition hover:-translate-y-0.5 hover:bg-black hover:shadow-[0_16px_36px_rgba(23,19,13,.22)]">Je souhaite m’inscrire</Link>}<Link href={infoHref(session)} className="inline-flex flex-1 items-center justify-center rounded-full border border-academy-line bg-white px-5 py-3 text-center text-sm font-black text-academy-ink transition hover:-translate-y-0.5 hover:border-academy-gold/50 hover:bg-stone-50">{session.status === 'FULL' ? 'Liste d’attente / prochaines dates' : 'Demander des infos'}</Link></div>
    ...
    </article>; })}</div> : <div className="rounded-[2rem] border border-academy-gold/30 bg-gradient-to-br from-white via-academy-gold-soft/45 to-white p-8 text-center shadow-card"><p className="text-2xl font-black">{emptyTitle}</p><p className="mx-auto mt-3 max-w-xl text-academy-muted">{emptyText}</p><Link href={emptyHref} className="mt-6 inline-flex rounded-full bg-academy-gold px-5 py-3 text-sm font-black text-academy-gold-text">{emptyCta}</Link></div>}
  </section>;
}
