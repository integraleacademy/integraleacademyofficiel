...
export function PublicTrainingSessions({ sessions, title = 'Prochaines dates', intro = 'Dates, places et informations issues de l’administration.' }: { sessions: PublicTrainingSession[]; title?: string; intro?: string }) {
  ...
  return <section className="mx-auto max-w-7xl px-4 py-10 sm:py-12">
    ...
    ].filter(Boolean) as { label: string; value: string; nowrap?: boolean }[]; return <article key={session.id} className={`reveal overflow-hidden rounded-[1.5rem] border p-4 shadow-soft ${index === 0 ? 'border-academy-gold bg-gradient-to-r from-academy-gold-soft to-white shadow-card' : 'border-academy-line bg-academy-elevated'}`}>
      <div className="grid gap-4 lg:grid-cols-[minmax(0,.8fr)_minmax(0,1.6fr)] lg:items-center xl:grid-cols-[minmax(0,.75fr)_minmax(0,1.6fr)_minmax(17rem,20rem)]">
        ...
        <div className="flex min-w-0 flex-col gap-2 sm:flex-row lg:col-span-2 xl:col-span-1"><Link href={primaryHref(session)} className="inline-flex min-w-0 flex-1 items-center justify-center rounded-full bg-academy-ink px-4 py-2.5 text-center text-sm font-black leading-tight text-white transition hover:-translate-y-0.5">Je réserve cette session</Link><Link href={infoHref(session)} className="inline-flex min-w-0 flex-1 items-center justify-center rounded-full border border-academy-line bg-white px-4 py-2.5 text-center text-sm font-black leading-tight text-academy-ink transition hover:-translate-y-0.5">Demander des infos</Link></div>
      </div>
