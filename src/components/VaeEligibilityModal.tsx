...
export function VaeEligibilityModal(){
  ...
  return <>
    <button type="button" onClick={()=>setOpen(true)} className="inline-flex items-center justify-center rounded-full bg-academy-gold px-5 py-3 text-sm font-black text-academy-ink shadow-gold transition hover:-translate-y-0.5 hover:bg-yellow-300 focus:outline-none focus:ring-4 focus:ring-academy-gold/30">Tester mon éligibilité à la VAE</button>
    {open&&<div className="fixed inset-0 z-50 grid place-items-center bg-black/70 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Simulateur d’éligibilité VAE DESP">
      <div className="flex h-[90vh] w-full max-w-5xl flex-col overflow-hidden rounded-[2rem] bg-white shadow-[0_30px_90px_rgba(0,0,0,.35)] ring-1 ring-white/20">
        <div className="flex items-center justify-between gap-4 border-b border-academy-line p-4 sm:p-5">
          <div>
            <p className="text-xs font-black uppercase tracking-[.2em] text-yellow-700">VAE DESP</p>
        ...
        <iframe src={simulatorUrl} title="Simulateur d’éligibilité VAE DESP" className="min-h-0 flex-1 bg-white" />
      </div>
    </div>}
  </>
}
